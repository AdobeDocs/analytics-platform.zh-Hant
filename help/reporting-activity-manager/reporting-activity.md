...
1. 查看每個連線的使用情況資訊。 表格中顯示的資料代表頁面上次載入時連線的報告活動。
   ...
   | UI 元素 | 說明 |
   | --- | --- |
   | **[!UICONTROL 連線]** | 您正在監視其報告活動的連線。 |
   | **[!UICONTROL 資料視圖]** | 顯示所有使用此連線的資料檢視。 資料釋圖設定可能會增加報告請求的複雜性。 |
   | **[!UICONTROL 容量使用情況]** | 即時顯示已使用的連線報告容量百分比。 <p>**請注意** 使用容量達到 100%，不一定表示您應該立即開始取消報告請求。 如果平均等待時間合理，100% 的使用容量可能屬於健康狀態。 另一方面，如果已佇列的請求數量也在增加中，則 100% 的使用容量可能表示有問題。</p> |
...
1. 選取您要查看詳細資訊之連線的連結標題。

   顯示您選取的連線報告活動資料。

1. (選用) 當連線首次載入報告活動管理員時，顯示的資料代表目前的使用情況指標。 若要在初始載入後查看更新的量度，請選取「[!UICONTROL **重新整理**]」按鈕以手動重新整理此頁面。

   <!-- Need to update this screenshot: ![connection](assets/indiv-report-ste.png) -->

1. 使用可用的圖表和表格來了解連線中的報告活動。
...
| [!UICONTROL **應用程式**] | [!UICONTROL 報告活動管理器]支援的應用程式有： <ul><li>Analysis Workspace UI</li><li>Workspace 排程專案</li><li>Report Builder</li><li>產生器 UI：區段、計算量度、註解、客群等。</li><li>來自 2.0 API 的 API 呼叫</li><li>警報<li>完整表格匯出</li><li>與任何人共用連結</li><li>引導式分析</li><li>查詢 Analytics 報告引擎的任何其他應用程式</li></li></ul><p>**注意：** 如果此欄的值為&#x200B;[!UICONTROL **未知**]，這表示請求中繼資料不適用於此使用者。</p> |
| [!UICONTROL **使用者**] | 提出請求的使用者。 <p>**注意：** 如果此欄的值為&#x200B;[!UICONTROL **未知**]，這表示請求中繼資料不適用於此使用者。</p> |
| [!UICONTROL **專案**] | 儲存的 Workspace 專案名稱、API 報告 ID 等。 (後設資料可能因各種應用程式而異。)<p>**注意：** 如果此欄的值為&#x200B;[!UICONTROL **未知**]，這表示專案尚未儲存或請求中繼資料不適用於此使用者。</p> |
...
| [!UICONTROL **應用程式**] | [!UICONTROL 報告活動管理器]支援的應用程式有： <ul><li>Analysis Workspace UI</li><li>Workspace 排程專案</li><li>Report Builder</li><li>產生器 UI：區段、計算量度、註解、客群等。</li><li>來自 2.0 API 的 API 呼叫</li><li>警報<li>完整表格匯出</li><li>與任何人共用連結</li><li>引導式分析</li><li>查詢 Analytics 報告引擎的任何其他應用程式</li></li></ul> |
