...
由於Adobe Analytics和Customer Journey Analytics的運作方式大不相同，因此無法進行1:1欄對應。 由於每個Adobe Analytics和Customer Journey Analytics實作差異較大，這些差異會進一步加劇。

此參考資料旨在協助資料工程師將其以Adobe Analytics為中心的資料摘要工作流程逐欄調整為以Customer Journey Analytics資料摘要為基礎的工作流程。
...
代表瀏覽器的數值 ID。請參考`browser.tsv`查詢表。
...
Adobe Advertising 整合變數。指定行動電信業者。`carrier.tsv`動態查詢的索引鍵值。
...
以`c_color`欄的值為基礎的色階 ID。請參考`color_depth.tsv`查詢表。

+++

+++**`connection_type`**

表示連線型別維度的數值ID。 請參考`connection_type.tsv`查詢表。
...
代表訪客接觸的第一個反向連結之類型的數值 ID。請參考`referrer_type.tsv`查詢表。
...
JavaScript 版的查詢 ID，以 `j_jscript` 為依據。請參考`javascript_version`查詢表。
...
代表訪客語言的數值 ID。請參考`languages.tsv`查詢表。
...
+++**`mobile_id`**
如果使用者使用行動裝置，則為裝置的數值 ID。`mobile_attributes.tsv`動態查詢的索引鍵值。
...
代表訪客作業系統的數值 ID。依據 `user_agent` 欄而定。`operating_system.tsv`標準查詢與`operating_system_type.tsv`動態查詢的索引鍵值。
...
影像要求中傳送的點擊類型 (標準點擊、下載連結、自訂連結、退出連結)。請參閱頁面事件查閱。
...
代表螢幕解析度的數值 ID。用於熒幕解析度維度。 使用`resolution.tsv`查詢表。

+++

+++**`search_engine`**

代表將訪客反向連結至您的網站的搜尋引擎的數值 ID。用於搜尋引擎維度。 請參考`search_engines.tsv`查詢表。
...
可識別上次接觸管道維度的數值ID。 此 ID 的查詢可以在行銷管道管理員中找到。
...
可識別首次接觸管道維度的數值ID。 此 ID 的查詢可以在行銷管道管理員中找到。
...
代表造訪的首次反向連結的反向連結類型數值 ID。請參考`referrer_type.tsv`查詢表。

+++

+++**`visit_search_engine`**

代表造訪的第一個搜尋引擎的數值 ID。請參考`search_engines.tsv`查詢表。
