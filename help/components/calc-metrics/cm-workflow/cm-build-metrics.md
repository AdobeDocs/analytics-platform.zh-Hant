...
1. 若要驗證您的計算量度定義是否正確，請使用不斷更新的計算指標結果&#x200B;**[!UICONTROL 預覽]** 。 **[!UICONTROL 預覽]**&#x200B;涵蓋過去 90 天的情況，並持續評估計算量度的定義。
   ...
   **[!UICONTROL 產品相容性]**&#x200B;指出計算量度是否可用於實驗和完整資料表匯出。 可能的值包括：
   * **[!UICONTROL 在 Customer Journey Analytics 中各處]**：計算量度可用於整個 Customer Journey Analytics。
   * **[!UICONTROL 不相容於：]**
      * **[!UICONTROL Experimentation]**：計算量度可用於所有Customer Journey Analytics （Experimentation面板除外）。
      * **[!UICONTROL 完整資料表匯出]**：計算度量可用於所有Customer Journey Analytics，除非從Workspace匯出完整資料表。 匯出完整資料表時，並非所有函式都受支援。 如果您在匯出完整表格時需要包含計算量度，請使用支援的函式。 如需詳細資訊，請參閱[將完整資料表匯出至雲端](/help/analysis-workspace/export/export-cloud.md#unsupported-calculated-metric-functions)中的[不支援的計算量度函式](/help/analysis-workspace/export/export-cloud.md)。
