...
Customer Journey Analytics提供畫布來拖放維度、量度、區段和函式，以便根據容器階層邏輯、規則和運運算元建立自訂量度。 此整合性開發工具可讓您建置並儲存簡單或複雜的計算量度。
...
1. 指定以下詳細資料 (![Required](/help/assets/icons/Required.svg) 是必要的)：
   ...
   | **[!UICONTROL 限專案量度]** | 編輯為單一專案建立的計算量度時，此對話方塊頂端會顯示資訊方塊，如[為單一專案建立計算量度](/help/components/apply-create-metrics.md#create-calculated-metrics-for-a-single-project)中所述。 <p>如果您想要讓此計算量度可用於所有專案，請選取&#x200B;**[!UICONTROL 讓此量度可用於所有專案，並將其新增至您的元件清單]**。</p> |
   ...
   | **[!UICONTROL 說明]** | 提供區段的說明，例如`Calculated metric to define the conversion rate.`。不需要說明計算量度的公式，因為公式已可在[!UICONTROL 摘要]中自動使用。 |
   ...
   | **[!UICONTROL 定義]** ![必填](/help/assets/icons/Required.svg) | 使用[定義產生器](#definition-builder)定義您的區段。 |
...
您可以使用「定義產生器」拖放維度、量度、區段和函式，根據容器階層邏輯、規則和運運算元建立自訂量度。 在該建構中，您可以使用標準量度、Adobe定義的量度、計算量度、區段、維度和函式。 計算量度產生器中的元件面板都提供所有這些元件。此外，您還可以在定義中使用運算子和容器。
...
* 將![事件](/help/assets/icons/Event.svg) **[!UICONTROL 量度]**&#x200B;元件從元件面板拖放到&#x200B;**[!UICONTROL 將量度、維度、維度專案、區段和/或函式拖放到這裡]**。 您可以使用元件列中的「![搜尋](/help/assets/icons/Search.svg)」來搜尋特定元件。
...
您可以將維度、區段和函式新增為容器至計算量度定義。 您也可以新增通用容器。容器的功能類似數學運算式，用以決定運算順序。容器內的一切都會在下一個元件或容器之前獲得處理。


#### 區段容器

您使用區段容器的概念來建立[分段量度](metrics-with-segments.md)。 您可以使用區段或維度建立的區段來建構區段容器。
...
使用區段名稱，自動將區段容器新增至**[!UICONTROL 定義]**。
  ...
  容器中的區段會以區段元件命名。 例如， ![分段](/help/assets/icons/Segmentation.svg) **[!UICONTROL 網頁工作階段]**。選取![資訊大綱](/help/assets/icons/InfoOutline.svg)以顯示包含區段詳細資訊的快顯視窗。 在快顯視窗中選取![編輯](/help/assets/icons/Edit.svg)以編輯區段定義。
...
* 拖放：
   ...
   1. 將![函式](/help/assets/icons/Effect.svg) **[!UICONTROL 函式]**&#x200B;元件從元件面板拖放到&#x200B;**[!UICONTROL 將量度、維度、維度專案、區段和/或函式拖放到這裡]**。 您可以使用元件列中的「![搜尋](/help/assets/icons/Search.svg)」來搜尋特定函數。
