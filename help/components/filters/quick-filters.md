...
1. 調整下列任一設定：
   ...
   | [!UICONTROL 點擊/造訪/訪客]容器 | 快速篩選僅包含一個[篩選容器](https://experienceleague.adobe.com/docs/analytics-platform/using/cja-components/cja-filters/filters-overview.html#filter-containers)，可讓您在篩選中包含（或從中排除）維度/量度/日期範圍。[!UICONTROL 訪客] 包含個人在造訪和頁面檢視間專屬的總體資料。 A [!UICONTROL 造訪] 容器可讓您設定規則，以根據造訪劃分人員的資料，並且 [!UICONTROL 點選] 容器可讓您根據頁面檢視來劃分人員資訊。 預設容器為[!UICONTROL 點擊]。 |
...
1. 套用快速篩選後，將滑鼠懸停在其上並選取資訊(「i」)圖示。
1. 選取「**[!UICONTROL 儲存篩選]**」
1. （可選）重新命名篩選，然後選取 [!UICONTROL **套用**].

   返回工作區並注意篩選的側邊欄從淺藍色變更為深藍色，表示在快速篩選產生器中無法再編輯或開啟它。 而透過將其儲存，該區段會成為元件清單的一部分。
