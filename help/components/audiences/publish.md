...
1. 建置對象。
   ...
   | [!UICONTROL 重新整理頻率] | 您想要重新整理對象的頻率。<ul><li>您可以選擇建立單次、不需重新整理的對象 (預設)。 例如，這可能適合用於特定單次行銷活動。</li><li>您也可以選擇其他重新整理間隔。 對於所有刷新頻率，限制為75或150個受眾，具體取決於您的CJA權利。 其他間隔則沒有對象數量上限。</li></ul> |
...
1. 解讀資料預覽。
   ...
   | [!UICONTROL 總人數] | 此對象中總人數的摘要數字。 它可以容納兩千萬人。 如果您的觀眾超過2000萬，則必須先縮小觀眾規模，然後才能發佈。 |
   | [!UICONTROL 對象規模限制] | 顯示這個觀眾的2000萬限制有多遠。 |
...
## 如果刪除CJA中的受眾，會發生什麼情況？ {#delete}

刪除CJA受眾後，該受眾將不再出現在Experience PlatformUI中。 但是，平台中沒有實際刪除與該訪問群體關聯的配置檔案。
