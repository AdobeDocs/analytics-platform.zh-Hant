---
title: 客戶自控金鑰
description: 了解如何為 Customer Journey Analytics 設定客戶託管金鑰。
...
1. 確保您有權使用 Adobe Customer Journey Analytics CMK，並且您的組織使用在 Azure 上執行的 Adobe Experience Platform。 您可以透過聯絡 Adobe 帳戶團隊來查看這些權利。
...
1. 授予 Adobe Azure 應用程式存取金鑰保存庫中金鑰的權限。 您可以使用下列其中一種方法來達成此目的：
...
1. Adobe 客戶服務部門確認已將 CMK 套用至您的 Customer Journey Analytics 資料。
...
如果您的組織使用在 Amazon Web Services 上執行的 Adobe Experience Platform，則已為您設定好 CMK。 無需額外設定。
