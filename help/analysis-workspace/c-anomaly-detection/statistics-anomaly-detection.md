...
對於每日詳細程度報表，演算法會考量數個重要因素，盡可能提供最準確的結果。首先，演演算法會根據我們在時間序列模型或極端值偵測模型（稱為功能分段）這兩種類別中選擇的可用資料，決定要套用的模型型別。
...
演算法會選取具有最佳平均絕對百分差 (MAPE) 者，測試上述每項的適合性。但是，如果最佳時間序列模型的MAPE大於15%，則會套用功能分段。 通常，資料高度重複者 (例如每週或每月重複) 最適合時間序列模型。
