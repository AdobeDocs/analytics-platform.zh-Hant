...
對於每日顆粒度報表，演算法會考量數個重要因素，盡可能提供最準確的結果。 首先，演演算法會根據演演算法從時間序列模型或極端值偵測模型（稱為功能分段）這兩種類別中選擇的可用資料，決定要套用的模型型別。
...
這些假日是根據對許多客戶資料點進行的廣泛統計分析而選出的，目的是找出對最多客戶趨勢最重要的假日。 此清單當然不是對所有客戶或業務週期都詳盡無遺，但套用假日可大幅改善幾乎所有客戶資料集的整體演演算法效能。
