...
>[!NOTE]
> 產品設定檔為管理哪些元件可向使用者顯示的主要機制。 透過[CX Enterprise Admin Console](https://experienceleague.adobe.com/zh-hant/docs/core-services/interface/administration/admin-tool-experience-cloud)管理。 組織為次要的區段。

## 套用專案組織

1. 按一下「**[!UICONTROL 共用]** > **[!UICONTROL 組織專案資料]**」。
專案中使用的元件將會自動新增。
如果專案有多個資料釋圖，您將看到專案中每個資料釋圖的策劃放置目標。
...
收件者開啟已策劃的專案時，只會看到您已定義的策劃元件集：


## 移除專案組織

若要移除專案策劃並還原左側面板中的完整元件集：
...
| **從資料檢視中新增或移除的元件** | 僅限新增到資料釋圖的元件 (隱藏或未隱藏)。 管理員不能報告資料視圖中未定義的欄位或元件。 | 僅限新增到資料釋圖的元件，或使用者擁有或與使用者共用的元件。 隱藏元件不適用 (例如虛擬報告套件監管)。 | 只有新增到資料視圖的元件未被隱藏，且已包含在專案組織中。 |
| **專案中已組織的元件** | 可供報告使用的所有資料視圖元件 (隱藏元件需要按一下「**[!UICONTROL 顯示全部]**」) | 所有非隱藏的資料釋圖元件 (需要按一下「顯示全部」) | 僅限已組織的元件，加上由用戶擁有或與用戶共用的元件 |
