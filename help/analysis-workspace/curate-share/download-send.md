...
如果您想要分析表格中超過 400 列的可見資料，請用滑鼠右鍵按一下表格標頭或任何一列，並選取「**將項目下載為 CSV 檔 (維度名稱)**」。 此選項將導出所選維的最多50,000個維項（基於表排序），並應用篩選器和篩選器。 如果您從表格最上方選擇此選項，將會匯出表格中的第一個維度。 當自由表格中未實施任何限制時，建議您在小於 20 欄的表格中使用「下載」項目選項，以確保最佳效能。
...
在上面的示例中，頁面項與面板篩選器（新訪問者客戶）和上面的元件（營銷渠道=電子郵件）一起下載作為篩選器，下面的元件（移動設備類型）則從下載的CSV中刪除。
