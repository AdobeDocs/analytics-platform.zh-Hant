---
description: 了解如何指定接觸點以建立多維度流失序列。
title: 設定流失視覺效果
...
您甚至可以在接觸點內新增篩選器。 例如，您可能想要比較篩選器，例如iOS和Android使用者。 將需要的篩選器拖曳至流失頂端，系統就會將這些篩選器的相關資訊新增至流失報表。 如果您只想顯示這些篩選器，可以移除「所有造訪」基線。
...
1. 繼續新增接觸點，直到完成您的序列。
   ...
   >[!NOTE]
   >
   >多個篩選器以AND連結，但多個項目（例如維度項目和量度）以OR連結。
