...
您甚至可以在觸點內添加濾鏡。 例如，您可能想比較篩選器，如iOS和Android用戶。 將所需過濾器拖到沈降片頂部，並將有關這些過濾器的資訊添加到沈降片報告。 如果只要顯示這些篩選器，可以刪除所有訪問基線。
