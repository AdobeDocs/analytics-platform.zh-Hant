...
1. （可選）根據所選內容建立篩選器。
   ...
   選擇單元格（連續或非連續），然後按一下右鍵> **[!UICONTROL 從所選內容建立篩選器]**。

1. 在 [篩選器生成器](/help/components/filters/manage-filters.md)，然後按一下 **[!UICONTROL 保存]**。

   保存的篩選器可用於 [!UICONTROL 篩選] 面板 [!UICONTROL Analysis Workspace]。
1. 命名同類群組專案，將其儲存。
