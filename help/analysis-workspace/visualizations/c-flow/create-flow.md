...
流量視覺效果可讓您了解源自或導致網站或應用程式上特定轉換事件的歷程。 它透過您的維度 (和維度項目) 或量度追蹤路徑。流量可讓您設定感興趣的路徑的起點或終點，或分析流經維度或維度項目的所有路徑。
...
如果停用「限制為首次/上次發生次數」，則這一系列點擊會計為2次「新增至購物車」。
預期流量輸出：
「加入購物車」(2) —>「產品」(1)
->「帳單」(1)

不過，如果啟用「限制為首次/上次發生次數」，則分析中只會包含「新增至購物車」的首次發生次數。
預期流量輸出：
「加入購物車」(1) —>「產品」(1)
