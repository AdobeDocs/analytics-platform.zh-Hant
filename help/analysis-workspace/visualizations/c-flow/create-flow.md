...
通過流可視化功能，您可以瞭解來自或通向網站或應用程式上特定轉換事件的旅程。 它透過您的維度 (和維度項目) 或量度追蹤路徑。流量可讓您設定感興趣的路徑的起點或終點，或分析流經維度或維度項目的所有路徑。
...
* 使用的系列因容器而異。如果使用 **[!UICONTROL 訪問]** 容器中，將會話作為一系列事件。 如果使用 **[!UICONTROL 訪問者]** 容器中，該系列事件將是給定用戶在所提供的日期範圍內的所有事件。
...
如果禁用「限制為第一次/最後一次出現」，則此單個系列事件將計數2次「添加到購物車」。
預期流量輸出：
「加入購物車」(2) —>「產品」(1)
->「帳單」(1)
...
如果 **[!UICONTROL 限制為第一次/最後一次出現]** 是 *禁用*，則此系列事件將計數2次「添加到購物車」。
預期流量輸出：
「產品」(2) &lt;—「加入購物車」(2)
