...
當您連線Journey Canvas中的節點時，會使用THEN運運算元將它們連線。 這也稱為[循序分段](/help/components/filters/seg-sequential-build.md)。
...
從歷程畫布建立的區段使用[循序分段](/help/components/filters/seg-sequential-build.md)。 這表示區段會使用THEN運運算元，將人們流經的事件（歷程）順序連結在一起，連至選取的節點或箭頭。 所有符合所選節點或箭號的事件都會納入區段中。
...
1. 指定區段的標題，並進行任何其他變更。 如需建立區段的詳細資訊，請參閱[區段產生器](/help/components/filters/filter-builder.md)。
