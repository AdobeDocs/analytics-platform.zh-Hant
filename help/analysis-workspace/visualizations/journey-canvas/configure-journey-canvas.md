...
1. 繼續依照下列各節所述自訂歷程。 您可以連接節點、重新命名節點、套用劃分、建立客群、新增時間限制等。
...
您可以選取節點，並顯示歷程中排名最前的即時或最終維度專案。 您可以將前 3 大維度項目新增至歷程畫布做為獨立節點，或在自由格式表格中檢視所有熱門的維度項目。
...
* 在畫布上，選取您要合併的節點，用滑鼠右鍵按一下其中一個選取的節點，然後選取&#x200B;[!UICONTROL **合併節點**]。
...
1. 在「歷程」畫布視覺效果中，執行下列任一項作業：
   ...
   * 以滑鼠右鍵按一下您要為其建立受眾之節點的流失（顯示流失時）。

   * 選取一或多個要建立對象的節點，然後以滑鼠右鍵按一下其中一個選取的節點。

   * 在2個要建立對象的節點之間選取一或多個箭頭，然後以滑鼠右鍵按一下其中一個選取的箭頭。
...
1. 選取&#x200B;[!UICONTROL **從節點建立區段**]、[!UICONTROL **從箭頭**]&#x200B;建立區段，或&#x200B;[!UICONTROL **從流失建立區段**]。
   ...
   接著會顯示區段產生器。 在&#x200B;[!UICONTROL **定義**]&#x200B;區段中，區段定義會根據您選取的節點、箭頭或流失及其在歷程中的內容來建立。
