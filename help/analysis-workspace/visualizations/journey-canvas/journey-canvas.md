...
您可以使用事件、維度專案、篩選器和日期範圍的任意組合來建立歷程節點，以建立使用者歷程的分析。 連線節點以建立歷程的流程，並包含多個路徑和決定點。 拖曳畫布上的節點以重新排列歷程的事件和條件。 變更時即時更新資料。
...
| 功能 | 優點 |
|---------|----------|
| **建立事件** | 使用任何Customer Journey Analytics維度、量度或篩選器，輕鬆建立事件。 <p>在Journey Optimizer中，技術使用者必須先建立事件，才能將其新增至歷程。</p> |
| **從自訂節點建立對象** | 根據您在Journey Canvas視覺效果歷程中建立的自訂節點建立對象。 (啟動Customer Journey Analytics對象產生器。) <p>在Journey Optimizer中，您只能為預先定義的活動建立對象。</p> |
| **流過和流失** | B3 |
| **劃分事件** | B3 |
| **重新命名事件** | B3 |
| **刪除事件** | B3 |
