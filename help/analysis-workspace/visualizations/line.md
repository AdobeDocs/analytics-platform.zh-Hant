---
description: 使用線條圖視覺效果來描繪趨勢（以時間為基礎）資料集
title: 折線圖
...
| 模型 | 說明 |
| --- | --- |
| 線性 | 為簡單的線性資料集建立最適合的直線，當資料以穩定速率增加或減少時相當實用。 方程式：`y = a + b * x` |
...
| 乘冪 | 建立曲線，對於比較以特定速率增加的測量資料集非常有用。 如果您的資料包含零或負值，則不應使用此選項。方程式：`y = a * x^b` |
| 二次方 | 找出拋物線形（向上或向下凹入）的資料集最合適的項目。 方程式：`y = a + b * x + c * x^2` |
