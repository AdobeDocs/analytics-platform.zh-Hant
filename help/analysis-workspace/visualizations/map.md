...
1. （視條件而定）如果您使用Web SDK，且已設定將經緯度填入資料流中，或如果您使用Analytics Source Connector填入事件資料，則經緯度欄位應該已可在您的架構中使用，並填入正確的內容標籤。

   找出這&#x200B;**[!UICONTROL Latitude]**&#x200B;和&#x200B;**[!UICONTROL Longitude]**&#x200B;結構描述欄位（在&#x200B;**[!UICONTROL 事件資料集]** > **[!UICONTROL placeContext]** > **[!UICONTROL geo]** > **[!UICONTROL _schema]**&#x200B;中），並將它們拖曳到您的資料檢視中，如果它們尚未出現，則做為維度。

   當這些結構欄位以維度形式存在於您的資料檢視中時，會自動套用其內容標籤，而地圖視覺效果會使用這些欄位，無需任何其他設定。

   ![新增經緯度結構描述欄位至資料檢視](assets/dataview-lat-long-default.png)

1. （視條件而定）如果您有想要用於經緯度資料的自訂維度，可以在自訂欄位上設定內容標籤：

   1. 在&#x200B;**[!UICONTROL 維度]**&#x200B;區段中，選取包含經度資料的維度。
   ...
   1. （選用）依預設，這些維度在地圖視覺效果中精確至城鎮或郵遞區號層級，在Workspace報表中顯示2位小數位數。 您可以在地圖視覺效果中，以精確的方式在單一米內調整它們，並在Workspace報表中顯示5個小數位數。 如需有關如何調整精確度等級的詳細資訊，請參閱[設定維度的精確位置](#configure-precise-locations-for-dimensions)。
...
1. （視條件而定）如果您使用網頁SDK，且已設定地理欄位填入資料流中，或如果您使用Analytics Source Connector填入事件資料，則地理欄位應該已可在您的結構描述中使用，並填入正確的內容標籤。
   ...
   ![將地理結構描述欄位新增至資料檢視](assets/dataview-geo-default.png)

1. （視條件而定）如果您有想要用於地理資料的自訂維度，可以在自訂欄位上設定內容標籤：
...
1. 選取您要用於緯度和經度的維度，以進行設定。 如需有關您使用哪些維度的詳細資訊，請參閱地圖視覺效果中[經緯度的必要內容標籤](#required-context-labels-for-latitude-and-longitude-in-the-map-visualization)。
