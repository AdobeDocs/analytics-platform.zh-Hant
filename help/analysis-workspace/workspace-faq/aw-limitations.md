...
* 當篩選器用作表格的列時，會停用從選取範圍建立量度。 此外，「從選取項目建立量度」不可套用至對齊日期的欄。
* 用於劃分列的條件式格式無法使用自訂範圍。
...
* 運用濾鏡的視覺化，例如[!UICONTROL 流失]、[!UICONTROL 流量]、[!UICONTROL 同類群組]和[!UICONTROL 色階分佈圖]，無法接受計算量度作為輸入。
...
* 如果計算量度是從「工作區」建立，則某些元件和運算子無法使用（與從「[!UICONTROL 元件>篩選器]」建立相反）。 例如 [!UICONTROL IP 位址]。
