...
# 優化 [!UICONTROL Analysis Workspace] 效能
...
| 自由格式儲存格數目 | 專案中自由格式儲存格的總數，以所有表格的列數 * 欄數計算。不包括隱藏的資料來源。建議為 4000 個。 | 將表格中的欄數減少至只剩最相關的資料點。通過調整顯示的行數、應用表篩選器或應用篩選器來減少表中的行數。 |
...
| 因素 | 定義 | 影響因素 | 最佳化 |
| --- | --- | --- | --- |
| 篩選複雜性 | 複雜的過濾器會對項目效能產生重大影響。 | 增加篩選器複雜性的因素（按影響的降序）包括： <ul><li>「包含」、「包含任何」、「符合」、「開始於」或「終止於」運算子 </li><li>順序篩選，特別是當使用尺寸限制（在/之後）時 </li><li>篩選器中使用的維中唯一維項的數目（例如，當頁有10個唯一項時，頁= &#39;A&#39;將比當頁有10000個唯一項時，頁= &#39;A&#39;快） </li><li>使用的不同維度數量 (例如：頁面 =「首頁」和頁面 =「搜尋結果」，其速度會比 eVar 1 =「紅色」和 eVar 2 =「藍色」更快)</li><li>多個「或」運算子 (而非「和」)</li><li>範圍各異的巢狀容器 (例如：「訪客」當中「造訪」內的「點擊」)</li></ul> | 雖然無法防止某些複雜因素，但請尋找機會來降低篩選器的複雜性。 通常，對於篩選條件，您可以越具體，越好。 例如：<ul><li>對於容器，在過濾器頂部使用單個容器比使用一系列嵌套容器更快。</li><li>若使用運算子，「等於」會比「包含」更快，而「等於任何」會比「包含任何」更快。</li><li>若使用多個條件，「和」運算子會比一系列「或」運算子更快。</li></ul> 盡可能將多個 OR 陳述式簡化為一個「等於任何」陳述式。<br> |
| 可視化複雜性（篩選器、度量、篩選器） | 專案本身新增的視覺效果類型 (例如，流失率與自由表格對比) 對專案效能的影響不大。視覺效果的複雜度會增加處理時間。 | 增加視覺效果複雜度的因素包括：<ul><li>要求的資料範圍</li><li>應用的篩選器數；例如，用作自由形式表行的篩選器</li><li>使用複雜濾鏡</li><li>[自由表格的靜態項目列或欄](/help/analysis-workspace/visualizations/freeform-table/column-row-settings/manual-vs-dynamic-rows.md)</li><li>自由表格列所套用的篩選器</li><li>包括的度量數，特別是使用篩選器的計算度量</li></ul> | 如果您注意到項目載入的速度沒有您想要的那麼快，請嘗試盡可能用eVars和篩選器替換某些篩選器。<br><br>如果您發現自己持續使用對您的業務至關重要的資料點的篩選器和計算度量，請考慮改進實施以更直接地捕獲這些資料點。 使用 Adobe Experience Platform Launch 之類的標記管理器和 Adobe 的處理規則，可以快速輕鬆地執行實作變更。 |
