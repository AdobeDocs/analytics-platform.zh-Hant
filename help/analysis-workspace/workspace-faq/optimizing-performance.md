...
## [!UICONTROL 說明] > [!UICONTROL 效能] 分析工作區中

在 **分析工作區> [!UICONTROL 說明] > [!UICONTROL 效能]**，您可以看到影響專案效能的因素，包括網路、瀏覽器和專案因素。 為獲得最準確的結果，請允許項目在開啟「效能」頁之前完全載入。

* 「目前的專案」欄會顯示您目前專案和使用者環境的結果。
* 「准則」欄會針對每個因素顯示Adobe的建議臨界值。

此外，您也可以 **下載為CSV** 可輕鬆與Adobe客戶服務或您內部的IT團隊分享效能內容。
...
[!UICONTROL 說明] > [!UICONTROL 效能] 項目因素包括：

| 因素 | 定義 | 最佳化 |
| --- | --- | --- |
| 查詢數 | 向Adobe提出以擷取專案中顯示之資料的查詢（請求）總數。 查詢包括表格的排名請求、異常偵測、Sparkline、左側導軌中顯示的元件等。 排除收合的面板和視覺化。 准則是100。 | 將資料分割為多個專案，以符合特定目的或利益相關者群組，盡可能簡化您的專案。 使用標籤將專案組織成主題，並使用 [直接連結](/help/analysis-workspace/curate-share/shareable-links.md) 建立內部目錄，讓利益相關者更輕鬆地找到所需內容。 |
