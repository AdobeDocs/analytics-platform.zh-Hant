...
| 網際網路頻寬 | 僅適用於 Google Chrome。您的瀏覽器對您所在位置的頻寬估計。 建議為 2.0 MB/s。 | 您的本機網路連線會影響此因素。 | 檢查您的本機網路連線。 |
...
| 已使用的元件 | 專案中使用的元件總數。建議為 100 個。 | 使用的元件數量不會直接影響效能。但這些元件的複雜度會影響專案效能。請參閱下方「其他因素」一節中的最佳化措施。 |
...
如需控管整體CJA效能的其他因素，請參閱 [最佳化CJA效能](/help/admin/cja-performance.md).