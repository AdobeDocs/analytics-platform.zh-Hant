...
>title="實驗"
>abstract="比較不同的使用者體驗、行銷或傳送訊息變化版本，確定哪一種最能達成特定結果。 指定實驗、作為比較對象的控制變體、成功量度和正規化量度。 或者，也可以設定信賴度的上限及下限。"


>[!BEGINSHADEBOX]

_本文記錄了_ ![CustomerJourneyAnalytics](/help/assets/icons/CustomerJourneyAnalytics.svg) _**Customer Journey Analytics**&#x200B;中的Experimentation面板。_<br/>_如需如何分析_ ![AdobeAnalytics](/help/assets/icons/AdobeAnalytics.svg) _**Adobe Analytics**&#x200B;中的Adobe Target活動和體驗的相關資訊，請參閱[Analytics for Target面板](https://experienceleague.adobe.com/zh-hant/docs/analytics/analyze/analysis-workspace/panels/a4t-panel)。_
...
所有 Customer Journey Analytics 的使用者都能使用實驗面板。 不需要管理員權限或其他權限。 然而，先決條件需要執行只有管理員才能進行的操作。
...
在包含實驗資料的資料釋圖中，選擇兩個維度，一個包含實驗資料，一個包含變體資料。 然後使用&#x200B;**[!UICONTROL 實驗中的實驗]**&#x200B;和&#x200B;**[!UICONTROL 實驗變體]**&#x200B;標籤標記這些維度。
...
1. 設定面板輸入設定：
   ...
   | 設定 | 定義 |
   | --- | --- |
   | **[!UICONTROL 日期範圍]** | 根據 Customer Journey Analytics 中收到所選實驗的第一個事件，系統會自動設定實驗面板的日期範圍。 如果需要，您可以將日期範圍限制或擴大至更具體的時間範圍。 |
...
您還將收到文字摘要，以表示實驗是否具有結論性，並總結結果。 結論性是以統計顯著性為基礎（請參閱[統計方法](#adobes-statistical-methodology)）。 您可以檢視具有最高提升度和信賴度的最佳表現變體摘要數字。
...
4. **提升度**：實驗報告摘要會顯示提升基線，這可測量指定變體的轉換率相對於基線的增進百分比。 準確來說，這是指定變體與基線之間的績效差異除以基線的績效，並以百分比表示。
...
>[!NOTE]
>
>對結果的完整描述應考慮所有可用的證據 (例如，實驗設計、樣本量、轉換率、信賴度及其他)，而不僅僅是宣告是否具有結論性。 即使結果尚未具有「結論性」，仍可能有令人信服的證據顯示某個變體與另一個變體不同 (例如，信賴區間幾乎不重疊)。 理想情況下，所有統計證據都應在連續光譜上加以解讀，並作為決策的依據。
...
例如，考慮一個人看到的廣告。 如果您決定顯示人員&#x200B;*廣告B*&#x200B;而非&#x200B;*廣告A*，可能會對測量某些量度的變更感興趣（例如平均收入）。顯示廣告B （而非廣告A）的因果關係，對達成行銷決策至關重要。 如果以展示廣告 B 的替代策略取代展示廣告 A 的現狀，那麼這種因果效應可以用整個人口的平均收入來測量。
...
Customer Journey Analytics 中的實驗面板可讓您分析不同人所在州的平均收入差異資料。 在這種情況下，結果不具有因果解釋。 然而，這樣的分析可能仍然有趣。 它提供了各州平均收入差異的估計值 (以及不確定性量度)。  該值也稱為&#x200B;*統計假設檢驗*。 這種分析的結果可能很有趣，但不一定具有可操作性。 只是因為您未隨機化，有時也無法將人們隨機化為維度的可能值之一。
...
>[!NOTE]
>
>對於同時使用 Customer Journey Analytics 和 Adobe Journey Optimizer 的組織，本節中的資訊也適用於 Journey Optimizer 中的實驗功能。
