...
| [!UICONTROL 量度] | 量度是有關個人活動的量化資訊，例如檢視、點進、重新載入、平均逗留時間、件數、訂購、收入等。 |
...
| [!UICONTROL 篩選] | 篩選器可讓您根據特性或網站互動來識別人員的子集。 例如，您可以建立 [!UICONTROL 訪客] 根據屬性的篩選器：瀏覽器型別、裝置、造訪次數、國家/地區、性別；或根據互動的篩選器：行銷活動、關鍵字搜尋、搜尋引擎；或根據退出點和登入點：來自Facebook的人員、定義的登陸頁面、反向連結網域；或根據自訂變數：表單欄位、定義的類別、客戶ID。 |
