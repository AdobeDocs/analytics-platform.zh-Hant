...
「媒體同時檢閱者」面板可啟用長時間進行的同時檢閱者分析，並可提供尖峰同時檢閱的詳細資訊，以及具備分項和比較的能力。若要存取「媒體同時檢閱者」面板，請導覽至已啟用Media Analytics元件的資料檢視。 然後，按一下最左側的面板圖示，並將面板拖放到您的 Analysis Workspace 專案中。
...
| 量度 | 說明 |
|---|---|
| 同時檢閱者 | 在特定時間點查看您媒體流的不重複訪客數量，而不計工作階段數量。<br><br>這與「報告」部分中使用「同時作用中工階段」的「同時檢閱者」報告不同。使用不重複訪客會移除顯示界限（工作階段會結束且同時開始）處不想要的「尖峰」。 |
