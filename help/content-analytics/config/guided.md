...
* 使用現有的Web標籤屬性或建立新的Web標籤屬性。
   ...
   * 若要建立新的Web標籤屬性：
     ...
     如果您想要為Web Channel建立Content Analytics的Tags [不可知的實作](/help/content-analytics/config/tags-agnostic.md)，請使用新的Tags屬性。 Tags屬性已建立，但您無法在不可知的實作中使用屬性。 不過，不可知的實作需要您執行引導式設定精靈至少一次。
