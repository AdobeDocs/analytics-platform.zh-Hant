...
description: 將Adobe Content Analytics設定為獨立解決方案的逐步指南。 瞭解如何設定結構描述、資料集、資料串流和報告，以在不實施完整Customer Journey Analytics的情況下獲得全面的內容效能深入分析。
...
* 基本瞭解網頁和行動分析概念、熟悉標籤管理系統，並具備JavaScript基本知識。 針對行動裝置頻道的Content Analytics，您應該具備行動應用程式開發技能。
...
| **Connection** | [連線](https://experienceleague.adobe.com/en/docs/analytics-platform/using/cja-connections/overview)定義要擷取的Experience Platform資料集。 連線會定義資料集（資料儲存在AEP中的位置）與Customer Journey Analytics （資料分析位置）之間的連結。 連線可讓您的收集資料可用於報表。 |
...
**階段1** - [準備您的環境](#prepare-your-environment)。 在此階段中，您需要設定使用者許可權並驗證您的資料基礎結構。 有了這些適當的許可權和資料結構，您就可以完成其餘步驟。 相關步驟包括：

1. **設定存取控制與許可權**&#x200B;以支援Content Analytics組態與實作。
1. **設定結構描述和資料集**&#x200B;以定義您要收集內容分析深入分析的資料模型（結構描述），以及收集該資料（資料集）的位置。

**階段2** - [設定資料集合](#configure-data-collection)。 在此階段中，您將建立從網站擷取內容資料的管道。 因此，Content Analytics知道訪客與哪些內容互動。
...
本節記錄您需要對產品、產品設定檔的存取權，以及設定和設定獨立Content Analytics所需的許可權。 雖然您只對Content Analytics的功能感興趣，但若要讓此功能正常運作，您仍需要其他Experience Platform產品的存取權和許可權。
...
請參閱![VideoCheckedOut](/help/assets/icons/VideoCheckedOut.svg) [管理產品設定檔的使用者](https://experienceleague.adobe.com/zh-hant/docs/experience-platform/access-control/ui/users){target="_blank"}以取得示範影片。
...
Experience Platform中的[!UICONTROL 許可權]介面是以角色的定義為基礎。 角色是資源型許可權的集合。 在新布建的環境中，有兩個預設角色可供使用： **[!UICONTROL 預設的生產所有存取權]**&#x200B;和&#x200B;**[!UICONTROL 沙箱管理員]**。
...
1. 啟用&#x200B;**[!UICONTROL 許可權]**&#x200B;的pin ![PinOn](/help/assets/icons/PinOn.svg)，因此&#x200B;**[!UICONTROL 快速存取]**&#x200B;中的&#x200B;**[!UICONTROL 許可權]**&#x200B;可供日後使用。
1. 選取&#x200B;**[!UICONTROL 許可權]**。
...
1. 在&#x200B;**[!UICONTROL 詳細資料]**&#x200B;畫面中：
   ...
   1. 若要新增缺少的資源，請從&#x200B;**[!UICONTROL 資源]** > **[!UICONTROL Adobe Experience Platform]**&#x200B;左側邊欄中選取&#x200B;**[!UICONTROL 資源名稱]** ![新增](/help/assets/icons/Add.svg)。
...
Customer Journey Analytics不支援屬性式存取控制。 若要指定許可權，請使用Admin Console。
...
若要從您的網站收集資料以供Content Analytics深入分析，您必須先定義您要收集哪些資料。 您也需要定義該資料的儲存方式。 在[透過Adobe Experience Platform Web SDK擷取資料](/help/data-ingestion/aepwebsdk.md)和[透過Adobe Experience Platform Mobile SDK擷取資料](/help/data-ingestion/aepmobilesdk.md)快速入門手冊[中設定結構描述和資料集](/help/data-ingestion/aepwebsdk.md#set-up-a-schema-and-dataset)中說明了這兩個概念。
...
您已定義要收集哪些資料以及如何儲存該資料。 下一個步驟是確保將從網站收集而來的資料路由傳送到資料集。 您需要設定資料串流，如[透過Adobe Experience Platform Web SDK](/help/data-ingestion/aepwebsdk.md)快速入門手冊擷取資料[設定資料串流](/help/data-ingestion/aepwebsdk.md#set-up-a-datastream)中所述。


### 使用標籤

您已定義要收集哪些資料（結構描述）、如何儲存該資料（資料集），以及從您的網站收集到的資料如何路由傳送到資料集（資料流）。 下一步，您需要標籤網站，以針對網站上資料層中的資料設定規則和資料元素。 標籤網站可確保將資料傳送至已設定的資料流。 使用標籤標籤標籤您的網站在[網頁SDK](/help/data-ingestion/aepwebsdk.md#use-tags)和[行動SDK](/help/data-ingestion/aepmobilesdk.md#use-tags)快速入門手冊的「使用標籤」中已有說明。
...
若要就收集的資料製作報表並為Content Analytics設定該資料，您需要在Customer Journey Analytics中設定連線。 連線會連線至包含所收集資料的資料集。 請參閱[網頁SDK](/help/data-ingestion/aepwebsdk.md)和[行動SDK](/help/data-ingestion/aepmobilesdk.md#set-up-a-connection)快速入門手冊中的[設定連線](../../data-ingestion/aepwebsdk.md#set-up-a-connection)。
...
使用[引導式設定精靈](guided.md)，並選取您在[設定資料檢視](#set-up-a-data-view)步驟中建立的資料檢視。 該選擇可確保在您從網站和行動應用程式收集資料的基礎上，設定並實施Content Analytics。
