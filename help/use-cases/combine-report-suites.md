...
的 [分析源連接器](https://experienceleague.adobe.com/docs/experience-platform/sources/ui-tutorials/create/adobe-applications/analytics.html?lang=zh-Hant) 提供了將Adobe Analytics的報告套件資料帶入Adobe Experience Platform供AEP應用程式（如RTCDP和CJA）使用的方法。 引入AEP的每個報告套件都配置為單個源連接資料流，每個資料流連接都配置為AEP資料湖中的資料集。 （分析源連接器將為每個報告套件建立一個資料集。）
...
這種情況將導致第1和第2eVar的報告毫無意義：

- eVar欄位將包含具有不同語義含義的值的混合。
- 搜索詞將分發在eVar1和eVar2之間。
- 將不可能對搜索條款、業務單位和客戶類別中的每個類別使用不同的屬性模型。

## 使用AEP資料準備解決報表套件之間的架構差異，以便在CJA中使用

AEP的資料準備功能與分析源連接器整合，可用於解決上述方案中描述的架構差異，從而在CJA資料視圖中產生具有一致含義的eVar。 下面是如何實現此目標的說明。 可以根據您的需要自定義下面使用的命名約定。

在為報表套件A和報表套件B建立源連接資料流之前，請在AEP中建立一個自定義欄位組（我們將調用它） **統一欄位** 在示例中)，其中包含以下欄位：
...
在AEP中建立新架構(我們將稱其為 **統一架構** 以我們為例。) 將以下欄位組添加到架構：
...
建立源連接資料流時 **報告套件A**&#x200B;選中 **統一架構** 用於資料流。 按如下方式添加自定義映射：
...
注：目標欄位的XDM路徑將取決於您如何設定自定義欄位組。

建立源連接資料流時 **報告套件B**&#x200B;中，選擇 **統一架構** 用於資料流。 工作流將顯示兩個欄位存在描述符名稱衝突。 這是因為，在報告套件B中，eVar1和eVar2的描述符與在報告套件A中的描述符不同。但我們已經知道了這一點，因此我們可以安全地忽略衝突並使用以下自定義映射：
...
立即建立 **所有報表套件** 資料集A和資料集B組合。

建立 **全局視圖** 資料視圖。 忽略原始eVar欄位，並僅包括「統一欄位」欄位組中的欄位。
...
實際上，您現在已將eVar1和eVar2從源報告套件映射到三個新欄位。 請注意，使用資料準備映射的另一個優勢是，目標欄位現在基於語義有意義的名稱（搜索術語、業務單元、客戶類別），而不是意義不大的eVar名稱(eVar1、eVar2)。

注：「統一欄位」自定義欄位組和關聯欄位映射可以隨時添加到現有分析源連接器資料流和資料集中，但這將僅影響向前資料。
...
為建立資料流時 **資料集A**&#x200B;選中 **擴展架構** 作為架構。 按如下方式添加自定義映射：

| 資料集A源欄位 | 「客戶資訊」欄位組中的目標欄位 |
| --- | --- |
| \_experience.analytics.customDimensions.eVars.eVar2 | _\&lt;path>_.Customer_category |

為建立資料流時 **資料集B**&#x200B;中，選擇 **擴展架構** 作為架構。 按如下方式添加自定義映射：
...
更改元件ID是僅CJA的函式，不會影響從分析源連接器發送到統一配置檔案和RTCDP的資料。
