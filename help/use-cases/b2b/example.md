...
| B2B個人資料集 | B2B個人綱要 | 輪廓 | XDM 個人輪廓 | XDM個人資料會形成已識別和部分識別個人的屬性和興趣的單一表示。 識別較少的設定檔可能僅包含匿名行為訊號，例如瀏覽器Cookie，而高度識別的設定檔可能包含詳細的個人資訊，例如姓名、出生日期、位置和電子郵件地址。 隨著個人檔案成長，它會成為個人資訊、身分資訊、聯絡詳細資料和個人通訊偏好設定的健全存放庫。 |
...
![已註解的B2B結構描述](assets/b2b-schemas-annotated.png)
...
1. 針對您想要針對以人員為基礎的B2B查詢轉換的每個查詢資料集，請啟用&#x200B;**[!UICONTROL 轉換資料集]**，以確保針對以人員為基礎的查詢轉換資料。 如需詳細資訊，請參閱[轉換B2B查詢的資料集](/help/connections/transform-datasets-b2b-lookups.md)。
   ...
   下表提供每個資料集的[!UICONTROL 人員ID]、[!UICONTROL 索引鍵]和[!UICONTROL 相符的索引鍵]範例值的範例概觀。

   >[!IMPORTANT]
   >
   >下表中&#x200B;**人員ID**、**金鑰**&#x200B;和&#x200B;**相符金鑰**&#x200B;的值是&#x200B;**範例值**，而且可以在您的特定環境中不同。
   >


   | 資料集（選擇性） | 個人 ID | 索引鍵<br/> | 比對索引鍵<br/>（在事件資料集中）<br/> |
   |---|---|---|---|
...
>[!IMPORTANT]
>
>下表中的量度及其值（**元件名稱**、**資料集**、**資料集型別**&#x200B;和&#x200B;**[!UICONTROL 結構描述路徑]）**&#x200B;為&#x200B;**範例**。 針對您的特定情況定義相關的B2B量度（元件名稱、資料集、資料型別和結構描述路徑）。
>

| 元件名稱 | 資料集 | 資料類型 | 結構描述路徑 |
|---|---|---|---|
| 年度帳戶收入 | B2B帳戶資料集 | 雙精度浮點數 | accountOrganization.annualRevenue.amount |
| 員工人數 | B2B帳戶資料集 | 整數 | accountOrganization.numberOfEmployees |
| 實際行銷活動成本 | B2B促銷活動資料集 | 雙精度浮點數 | actualcost.amount |
| 預算行銷活動成本 | B2B促銷活動資料集 | 雙精度浮點數 | budgetedCost.amount |
| 預期機會收入 | B2B機會資料集 | 雙精度浮點數 | expectedRevenue.amount |
| 預期行銷活動收入 | B2B促銷活動資料集 | 雙精度浮點數 | expectedRevenue.amount |
| 機會金額 | B2B機會資料集 | 雙精度浮點數 | opportunityAmount.amount |
...
>[!IMPORTANT]
>
>下表中的維度及其值（**元件名稱**、**資料集**、**資料集型別**&#x200B;和&#x200B;**[!UICONTROL 結構描述路徑]）**&#x200B;為&#x200B;**範例**。 針對您的特定情況定義相關的B2B維度（元件名稱、資料集、資料型別和結構描述路徑）。
>
