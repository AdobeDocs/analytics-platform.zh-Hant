...
品牌通常將整體轉換率視為成功的落後指標，並專注於轉換率最佳化以增加曝光次數。 但轉換率是所有虛榮量度的王；這可能表示短期獲勝，但透過將數量優先於客戶體驗品質，您可能會對品牌造成長期損害。

## 使用案例概觀

客戶轉換最佳化使用案例的主要目標是將目前處於考量階段的前景客戶推進為轉換的客戶。 若要這麼做，您必須考量每位客戶的轉換需求，而非只檢視彙總的轉換率。

### 與舊版一起離開

許多組織不斷受到挑戰，需要瞭解巨集層級的轉換，例如創收事件，然後搭配任何型別的轉換事件的成因。 微觀和奈米區段客戶很難針對不同同類群組推出目標式行銷活動，以便促進註冊和轉換。

典型的轉換最佳化策略也僅限於Web或行動等數位頻道。 由於缺乏客戶在其他接觸點互動的知識，品牌很難規劃有影響力的多頻道體驗，以引導個人從考量到轉換。 因此，他們可能會繼續傳送無關緊要或過時的訊息，進一步冒著品牌長期受損的風險。

### 使用新的

透過客戶轉換最佳化，品牌可以將目前處於考量階段的前景客戶轉換為已轉換的客戶。 連線轉換最佳化工具會一對一分析每個潛在客戶的體驗，並據此改善其獨特體驗。 專注於每位使用者的旅程，並專注於個別客戶的成功，您可以：

* 透過特徵和行為資料來分析引人入勝體驗的所有情境驅動因素，以改進轉換事件。
* 針對尚未註冊或轉換的假名設定檔，使用進階分段和啟動工具。
* 跨廣告、社交、電子郵件、文字等擴充一對一體驗，使用興趣和偏好的管道建立個人化。
* 使用AI模型來識別指標，並將使用者路由到最佳路徑，例如免費試用或聊天。

客戶轉換最佳化就是關於平衡。 短期成功量度（例如轉換率）很重要，但長期量度（例如忠誠度、終生價值、品牌情緒和淨推廣者分數）可確保您的品牌擁有光明和可持續的未來。

因此，與其嘗試從廣泛的細分市場產生相同的轉換事件，不如專注於建立持續的一對一客戶關係，並分析成功的訊號和模式。 以客戶體驗為中心的策略，其副產品轉換率自然會有所改善。

### 目標、挑戰與差異

客戶轉換最佳化使用案例可解決在嘗試達成目標時的一些常見挑戰。 Adobe解決方案(如Real-time Customer Data Platform、Journey Optimizer和Customer Journey Analytics)及其基礎功能的組合，使您在克服這些挑戰時與眾不同。

| 目標 | 挑戰 | 差異 |
|---| ---|---|
| **建置深度 <br/>瞭解<br/>每個人<br/>潛在客戶歷程<br/>和體驗。** | 大部分組織都面臨瞭解巨集層級轉換（創收事件）的挑戰。 然後將該巨集層級轉換與任何型別轉換事件的貢獻因素配對。<br/><br/>轉換最佳化也僅限於單獨的數位頻道（例如Web或行動裝置）。 | Adobe提供更全面、以客戶為中心的解決方案，讓品牌可分析所有有助於提供正面且吸引人體驗的情境式動因，以針對各個管道的每個客戶，推動轉換事件的超大改善。<br/><br/>您可在整個歷程中執行深入分析，使用特徵和行為資料來最佳化體驗 |
| **識別和分析<br/>訊號與意圖<br/>針對每個潛在客戶<br/>轉換需求。** | 難以對微和奈米區段客戶針對不同同類群組啟動鎖定目標的行銷活動，以促進註冊或轉換<br/><br/>品牌很難使用匿名行為來計畫跨管道的有影響力的體驗。 | Adobe可為尚未註冊或轉換的假名設定檔提供進階分段和啟動功能。<br/><br/>由於使用者在轉換的考量過程中取得了進展，Adobe應用程式可以共同合作，跨廣告、社交、電子郵件、文字等內容建立連線的全通路體驗。 |
| **參與和<br/>個人化至每個<br/>潛在客戶相應<br/>按比例縮放。** | 缺乏客戶在其他管道互動的知識，導致訊息不相關或過時，並可能對品牌造成長期經濟損害 <br/><br/>當使用者經過考量轉換的各個階段時，就很難維持一致且連結的體驗 | 協助一對一個人化全通路行銷，確認客戶的興趣，並讓客戶透過其偏好的通路參與，以開始客戶使用您品牌的歷程<br/><br/>使用AI模型來聰明地識別指標，並將每位使用者路由到最佳路徑，例如試用下載或聊天。 |
...
若要實現並實作您的特定客戶轉換最佳化使用案例，請考慮使用下列Adobe解決方案。

### Customer Journey Analytics

Customer Journey Analytics可協助您瞭解哪些因素可驅動、阻止及說服客戶達成關鍵成果。 Customer Journey Analytics利用對完全相關資料母體進行深入且彈性的Ad Hoc分析，即時瞭解並因應客戶需求。 Customer Journey Analytics可原生將深入分析連結至Real-time Customer Data Platform和Journey Optimizer，以建立最佳化策略，持續改善您的業務。

#### 架構

若要瞭解Customer Journey Analytics如何融入Adobe解決方案和您基礎建設的整體架構，請參閱
...
| 功能 | 說明 | 開始使用…… |
|---|---|---|
| 適用於Customer Insight的Analysis Workspace。 | 根據使用者角色量身打造的彈性工作區，用於深入資料探索。 任何負責客戶歷程的人，都可以根據深入的客戶洞察力，做出資料導向式決策。 | <ul><li>[Analysis Workspace 概觀](https://experienceleague.adobe.com/en/docs/analytics-platform/using/cja-workspace/home)</li></ul> |
...
Real-time Customer Data Platform可協助您對卡在考量階段的使用者進行細分並加以啟用。 Real-time Customer Data Platform運用進階分段功能和AI來分析行為資料。 您可以針對最佳體驗機會取得傾向分數，以推動註冊和轉換。

#### 架構

若要瞭解Real-time Customer Data Platform如何融入Adobe解決方案和您基礎建設的整體架構，請參閱下列架構藍圖：
...
若要瞭解Journey Optimizer如何融入Adobe解決方案和您基礎建設的整體架構，請參閱下列架構藍圖：
...
以下範例說明虛擬品牌Luma和名為Sarah的虛擬客戶的客戶轉換最佳化使用案例。

| 步驟 | 說明 | 更多資訊 |
|---|---|---|
| 1 | Sarah是Luma Plus會員，曾線上上和店內進行購買。 最近，她沒有看到任何讓她感興趣的產品，也沒有購買過任何產品。 | |
| 2 | 您希望識別像Sarah這樣的客戶。 先前已購買瑜伽服飾，但最近未購買的客戶。 | 使用Customer Journey Analytics來定義已購買瑜伽產品但最近未購買的客戶的篩選器（區段）。<br/>將篩選器套用至行銷活動自由表格，以識別為該群組帶來最多銷售額的行銷活動。 |
| 3 | 您也想要向新訪客和潛在客戶行銷 | 使用Customer AI ，針對可能購買瑜伽服飾並建立區段的客戶，使用Customer Upsell模型來執行傾向分數。 在即時客戶資料設定檔中，檢閱區段及其選項。 啟用區段至Target。 |
| 4 | 莎拉正在看晨報，並觀看Luma瑜伽廣告。 她接受Cookie、點選廣告並發現網站已針對她個人化（使用Target）。 她開始瀏覽內容，每次點按內容就會逐步個人化。 | 您可以使用Target建立個人化訪客體驗的活動。 |
| 5 | Sarah將瑜伽短褲加入購物車……並提供電子郵件地址登入。 | 您注意到即時客戶資料設定檔將可識別Sarah （把她從匿名移至已知設定檔）。 |
| 6 | 由於對產品本身不確定、價格高昂，而且在購買產品前找不到Sara所需的資訊，她決定致電客服中心尋求協助。 客服中心代表會提供她一些其他詳細資料，說明她有資格在附近的Luma參加Luma免費瑜伽課程。 | 您使用與Real-time Customer Data Platform整合的客服中心應用程式，在交談時尋找有關Sarah的所有相關資訊。 <br/>您可以使用決定管理來設定決定、位置和優惠。 |
| 7 | 在接聽客服中心代理的詳細資訊並享受10%折扣後，Sarah決定完成購買。 她收到電子郵件確認函，確認信件會顯示所購買產品的資訊、更新的忠誠度點狀態，以及在附近Luma的免費瑜伽課程優惠。 | 您可以使用Real-time Customer Data Platform中的設定檔檢視器來檢視Sarah的設定檔，以及她現在符合資格的任何新區段會籍。 <br/>您使用Journey Optimizer來建置歷程，以便在購買時傳送確認電子郵件。<br/>您可以使用決定管理來定義規則，這些規則會傳送特定優惠作為確認電子郵件的一部分。 |
| ... | 等等。 | |
