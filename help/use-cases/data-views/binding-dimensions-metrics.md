...
您可以將物件陣列中的維度項目繫結到另一個維度。 當繫結維度專案出現時，Customer Journey Analytics會重新呼叫繫結維度，並將其包含在事件中。 考量下列客戶歷程：
...
當您設定此持續性模型時，每當設定了產品顏色時，Customer Journey Analytics都會記下產品名稱。 當它在後續事件中為該人員識別出相同的產品名稱時，產品顏色也會隨之改變。 當您將產品顏色繫結到產品名稱時，相同的資料會類似於以下內容：
...
雖然此範例僅包含一個使用者，但搜尋不同東西的許多人可能會將搜尋字詞錯誤地歸因於不同的產品，導致難以判斷最佳搜尋結果到底是什麼。
