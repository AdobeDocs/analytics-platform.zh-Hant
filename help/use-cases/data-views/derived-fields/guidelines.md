...
* [高基數衍生欄位](#high-cardinality-derived-fields)
* [規則鏈結時過度複雜的大小寫](#over-complex-case-when-rule-chains)
* [使用錯誤](#wrong-usage)
* [量度和維度的分類錯誤](#misclassifications-of-metrics-and-dimensions)
* [行銷管道和行銷活動邏輯陷阱](#marketing-channel-and-campaign-logic-pitfalls)
* [用於查閱的非標準化字串索引鍵](#non-normalized-string-keys-used-in-lookups)
* [Regex濫用或伸手過長](#regex-misuse-or-overreach)
* [衍生欄位中的計算量度樣式邏輯](#calculated-metric-style-logic-in-derived-fields)
* [超量使用下一個或上一個或循序函式](#over-usage-of-next-or-previous-or-sequential-functions)
* [忽略工作階段和個人層級內容](#ignoring-session-and-person-level-context)
* [達到或接近記錄函式限制](#hitting-or-nearing-documented-function-limits)
* [資料檢視專用最佳化規則](#data-view-specific-optimization-rules)
...
* 簡單操作，例如[小寫](/help/data-views/derived-fields/derived-fields.md#lowercase)、[Trim](/help/data-views/derived-fields/derived-fields.md#trim)或[Case When](/help/data-views/derived-fields/derived-fields.md#case-when)檢查頁面URL通常比低基數欄位的相同邏輯更昂貴。
...
* 避免直接在資料檢視預設區段中參考完整頁面URL或類似的高基數元件。 將大量URL邏輯（複雜的[大小寫條件](/help/data-views/derived-fields/derived-fields.md#case-when)，[Regex取代](/help/data-views/derived-fields/derived-fields.md#regex-replace)，多個字串函式）向上推播至[資料準備](https://experienceleague.adobe.com/zh-hant/docs/experience-platform/data-prep/home)或[查詢資料集](/help/getting-started/cja-upgrade/cja-upgrade-dataset-lookup.md)，因此產生的分類會以較簡單、較低基數維度為基礎。
* 偏好較低基數索引鍵，例如標準化頁面名稱、網站區段或預先分類的URL群組。
* 定期稽核現有資料檢視預設區段和衍生欄位，以參考高基數維度（頁面URL、促銷活動ID、原始查詢字串），並重構為標準化或分組的索引鍵。

## 規則鏈結時過度複雜的大小寫

本節討論[Case When](/help/data-views/derived-fields/derived-fields.md#case-when)規則的過於複雜鏈結。

Customer Journey Analytics會針對每個衍生欄位強制執行明確的[函式與運運算元限制](/help/data-views/derived-fields/derived-fields.md#limitations) （例如，運運算元數目上限、每種型別的函式數目上限）。 過於複雜的函式以及函式中的鏈結更難維護，也更容易出錯。
...
* 潛在效能和限制風險：您可能會點選或接近[運運算元或函式限制](/help/data-views/derived-fields/derived-fields.md#limitations)，尤其是使用類似分類的模式。

**推薦**

* 分割成多個衍生欄位。 例如，將&#x200B;*行銷活動標準化* （將不一致的行銷活動識別碼對應到標準值）與管道分組分開，而不是在一個巨大規則中組合所有內容。
* 使用查詢資料集。 許多&#x200B;**[!UICONTROL 如果值&#x200B;_值_條件&#x200B;_條件_然後將&#x200B;_值_設定為值]**&#x200B;條件較適合實作為[查詢資料集](/help/getting-started/cja-upgrade/cja-upgrade-dataset-lookup.md)結合[查詢](/help/data-views/derived-fields/derived-fields.md#lookup)函式，而非使用長[Case When](/help/data-views/derived-fields/derived-fields.md#case-when)鏈結。
...
>[!NOTE]
>
>考慮上游簡化：使用[資料準備](https://experienceleague.adobe.com/zh-hant/docs/experience-platform/data-prep/home)、[查詢資料集](/help/getting-started/cja-upgrade/cja-upgrade-dataset-lookup.md)或衍生欄位函式（如[分類](/help/data-views/derived-fields/derived-fields.md#classify)）來合併類似的行銷管道規則，並減少[Case When](/help/data-views/derived-fields/derived-fields.md#case-when)邏輯中的運運算元數量。 此外，限制頻道分類邏輯中參考的高基數欄位數量（例如：許多不同的查詢引數索引鍵），因為這些欄位會增加基數和查詢成本。
...
* 事件或設定檔欄位上的[查詢](/help/data-views/derived-fields/derived-fields.md#lookup)函式，可饋送查詢資料集。
* 沒有前面的[小寫](/help/data-views/derived-fields/derived-fields.md#lowercase)、[Trim](/help/data-views/derived-fields/derived-fields.md#trim)或[Regex Replace](/help/data-views/derived-fields/derived-fields.md#regex-replace)將金鑰標準化。
...
* 在[Lookup](/help/data-views/derived-fields/derived-fields.md#lookup)函式之前新增[小寫](/help/data-views/derived-fields/derived-fields.md#lowercase)和[Trim](/help/data-views/derived-fields/derived-fields.md#trim)函式，除非有檔案說明保留大寫或小寫的原因。
...
* [Regex Replace](/help/data-views/derived-fields/derived-fields.md#regex-replace)或以Regex為基礎的條件使用廣泛的模式；較簡單的[Case When](/help/data-views/derived-fields/derived-fields.md#case-when)函式具有&#x200B;**[!UICONTROL Contains]**&#x200B;或&#x200B;**[!UICONTROL Starts with]**&#x200B;是較好的替代方案。
...
* 多個規則運算式條件重疊或衝突。
* 大量使用規則運算式來剖析URL，而非使用[URL Parse](/help/data-views/derived-fields/derived-fields.md#url-parse)函式。
...
* 偏好使用標準URL元素（網域、路徑、查詢引數）的[URL剖析](/help/data-views/derived-fields/derived-fields.md#url-parse)，而非[Regex取代](/help/data-views/derived-fields/derived-fields.md#regex-replace)。
* 對於簡單模式檢查，請使用[Case When](/help/data-views/derived-fields/derived-fields.md#case-when)搭配&#x200B;**[!UICONTROL Contains]**、**[!UICONTROL Starts with]**&#x200B;或&#x200B;**[!UICONTROL Ends with]**&#x200B;邏輯，而不是搭配[Regex Replace](/help/data-views/derived-fields/derived-fields.md#regex-replace)的規則運算式。
...
本節討論[下一個或上一個](/help/data-views/derived-fields/derived-fields.md#next-or-previous)或循序函式的過度使用。

**模式**

* 衍生欄位多次使用[Next或Previous](/help/data-views/derived-fields/derived-fields.md#next-or-previous)函式（接近記錄的每個欄位限制）。
* [下一個或上一個](/help/data-views/derived-fields/derived-fields.md#next-or-previous)用於實作持續性類似的邏輯（例如：繼續促銷活動），而不是使用資料檢視持續性。

**風險診斷：資料品質，高維護性**

* 複雜性與脆弱性：沈重的循序邏輯較難解釋，並且可能在工作階段化規則或順序變更時中斷。
* 具維度持續性的備援：維度上的資料檢視[持續性](/help/data-views/component-settings/persistence.md)設定（配置模式）能更好地涵蓋某些使用案例（例如工作階段上的上次接觸管道）。

**推薦**

* 對於類似標準持續性的模式（例如，跨工作階段或人員向前傳送值），請在資料檢視中使用維度的[持續性](/help/data-views/component-settings/persistence.md)設定（**[!UICONTROL 配置模式]**&#x200B;和&#x200B;**[!UICONTROL 有效期]**），而不是使用[下一個或上一個](/help/data-views/derived-fields/derived-fields.md#next-or-previous)模擬這些模式。
* 保留[下一個或上一個](/help/data-views/derived-fields/derived-fields.md#next-or-previous)，用於進階多步驟路徑或funnel標籤，其維度持續性無法單獨達成（例如：頻道序列串連）。
...
>[!NOTE]
>
>儘可能減少對複雜衍生欄位中高基數欄位的依賴（例如：使用標準化索引鍵或群組分類），以限制查詢成本及達到[運運算元或函式限制](/help/data-views/derived-fields/derived-fields.md#limitations)的可能性。

Customer Journey Analytics [檔案](/help/data-views/derived-fields/derived-fields.md#limitations)每個衍生欄位的最大函式和運運算元，包括每個函式型別的限制。atterns**

* 衍生欄位使用許多[查詢](/help/data-views/derived-fields/derived-fields.md#lookup)、[Math](/help/data-views/derived-fields/derived-fields.md#math)作業、[分割](/help/data-views/derived-fields/derived-fields.md#split)或其他函式。
* 運運算元數目接近[記錄的限制](/help/data-views/derived-fields/derived-fields.md#limitations) （例如：超過70% - 80%的允許計數）。
...
* 調整維度的[持續性](/help/data-views/component-settings/persistence.md)設定中的&#x200B;**[!UICONTROL 配置模式]**&#x200B;和&#x200B;**[!UICONTROL 有效期]**，以對齊名稱和行為。 例如，`Original Source`應該將&#x200B;**[!UICONTROL 配置模式]**&#x200B;設定為&#x200B;**[!UICONTROL 原始]**，並將&#x200B;**[!UICONTROL 到期]**&#x200B;設定為&#x200B;**[!UICONTROL 人員]**。
