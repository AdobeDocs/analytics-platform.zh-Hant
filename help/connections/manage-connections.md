...
| [!UICONTROL 略過的記錄] Widget | 針對您選取的資料集和日期範圍&#x200B;**，指出在選取的時段內已略過多少列。**&#x200B;略過記錄的原因包括：遺失時間戳記、遺失或無效的人員ID等。 每 10 分鐘更新一次。<p>**注意**：**[!UICONTROL 略過的記錄]**&#x200B;此時僅包括事件資料，不包括設定檔或查詢資料。<p>無效的人員ID（未定義，全部零）無法歸因於任何特定的使用者或人員。 無法將擷取資料擷取至系統中，且會導致擷取和報表程式發生錯誤。 若要修正無效的人員ID，您有3個選項：<ul><li>使用匯整功能，將未定義或全部為零的使用者ID填入有效的使用者ID。</li><li>將使用者ID留空，在擷取期間也會略過這些ID（偏好使用無效或完全零的使用者ID）。</li><li>在擷取資料之前，請先修正系統中任何無效的使用者ID。</li></ul> |
...
| [!UICONTROL 略過的記錄] | 指出在選取的時段內擷取期間已略過多少列。<p>**注意**：**[!UICONTROL 略過的記錄]**&#x200B;此時僅包括事件資料，不包括設定檔或查詢資料。<p>無效的人員ID（未定義，全部零）無法歸因於任何特定的使用者或人員。 無法將擷取資料擷取至系統中，且會導致擷取和報表程式發生錯誤。 若要修正無效的人員ID，您有3個選項：<ul><li>使用匯整功能，將未定義或全部為零的使用者ID填入有效的使用者ID。</li><li>將使用者ID留空，在擷取期間也會略過這些ID（偏好使用無效或完全零的使用者ID）。</li><li>在擷取資料之前，請先修正系統中任何無效的使用者ID。</li></ul> |
