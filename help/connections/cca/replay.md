...
不會重播後視窗口以外的資料。 訪問者必須在給定的回望窗口內進行身份驗證，才能同時識別未經身份驗證的訪問和經過身份驗證的訪問。 一旦識別到設備，就會從該點向前即時縫合。
...
| `6` | - | - | `Bob` | `Bob` | Bob再次呼叫客戶服務 | `3` (246、Bob 和 3579) |
...
| `2` | `246` | `Bob` | - | `Bob` | Bob 透過桌上型電腦登入 | `1` （鮑勃） |
| `3` | - | - | `Bob` | `Bob` | Bob致電客戶服務 | `1` （鮑勃） |
| `4` | `3579` | - | - | `Bob` | Bob 在未經驗證的情況下，透過行動裝置存取您的網站 | `1` （鮑勃） |
| `5` | `3579` | `Bob` | - | `Bob` | Bob 透過行動裝置登入 | `1` （鮑勃） |
| `6` | - | - | `Bob` | `Bob` | Bob再次呼叫客戶服務 | `1` （鮑勃） |
| `7` | `246` | - | - | `Bob` | Bob 在未經驗證的情況下，再次透過桌上型電腦造訪您的網站 | `1` （鮑勃） |
