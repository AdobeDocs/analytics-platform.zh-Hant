...
Analytics 控制面板可隨時隨地提供 Customer Journey Analytics 的深入分析。此應用程式可讓用戶透過行動裝置存取計分卡，享有符合直覺的操作方式。計分卡是關鍵量度和其他元件的集合，以圖磚式版面配置呈現，點選上面的項目即可取得詳細劃分資料和趨勢報表。iOS 和 Android 作業系統均支援控制面板。
