...
1. 在右側邊欄的&#x200B;**[!UICONTROL 「屬性」]**&#x200B;下方，指定您要使用資料的&#x200B;**[!UICONTROL 專案報表套裝]**。報表套裝是CJA中的資料檢視。
...
若要將篩選器套用至個別圖磚，請從左側面板拖曳篩選器（區段是CJA中的篩選器），並將其直接拖曳至圖磚上。
...
此視覺效果類似圓餅圖，以整體的形式顯示資料。 若要比較總數的百分比時，可使用環形圖。例如，假設您要了解哪個廣告平台對不重複訪客的總數有助益：
...
按一下計分卡建立程式中的圖磚時，右側邊欄會顯示與該圖磚及其詳細資料投影片相關聯的屬性和特性。 在此邊欄中，您可以提供新 **標題** 為圖磚，或透過套用篩選器來設定圖磚。 區段是CJA中的篩選器。

![「屬性」圖磚](assets/properties-tile-new.png)

## 查看詳細幻燈片 {#view-detail-slides}

按一下圖磚時，動態快顯視窗會顯示應用程式中執行使用者如何看到詳細資料投影片。 您可以新增維度，以根據您的特定需求劃分資料。 如果尚未套用維度，劃分維度將會 **小時** 或 **天數**，視預設日期範圍而定。

劃分會依維度項目劃分量度，如下所示，借此縮小分析範圍：
