...
**Analytics [!UICONTROL 控制面板]**&#x200B;可隨時隨地提供 Customer Journey Analytics 的洞察。 此應用程式可讓使用者透過行動裝置存取直覺式計分卡。 行動計分卡是關鍵量度和其他元件的集合，以圖磚式版面配置呈現，點選其中的項目即可取得更多詳細劃分資料和趨勢報告。 iOS 和 Android 作業系統均支援此行動應用程式。
...
* [高階主管使用者快速入門指南](/help/mobile-app/executive.md)：本指南旨在協助高階主管使用者閱讀和理解[!UICONTROL 儀表板]上的計分卡。 此應用程式可讓高階主管使用者在自己的行動裝置上，輕鬆快速地檢視重要摘要資料的整體概況。
