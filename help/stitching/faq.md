...
* **每週**&#x200B;重播會在每個&#x200B;**星期六**&#x200B;晚上（客戶時區）開始，且資料會在星期一早上前在Customer Journey Analytics報表中更新。
* **每日**&#x200B;重播會在上午&#x200B;**3點** （客戶時區）左右執行，並且資料會在早上之前在Customer Journey Analytics報表中更新。

>[!IMPORTANT]
>
>每週重播的當天和每日重播的小時無法透過自訂請求變更。
>
...
**問題**：一旦我連絡我的Adobe客戶團隊並提供所需資訊後，金鑰已重設的資料集需要多久才能使用？
...
**問題**：跨裝置分析（傳統Analytics中的一項功能）和跨管道分析之間有何差異？
...
**問題**：如果一個或多個事件中的永久ID欄位為空白，會發生什麼情況？
...
**問題**：如果一或多個事件中的人員ID欄位有預留位置值（如`Undefined`），會發生什麼情況？
...
**問題**： Customer Journey Analytics拼接資料集中的量度，與Customer Journey Analytics非拼接資料集中的類似量度以及與Adobe Analytics相比如何？
...
**問題**： Customer Journey Analytics可以使用「身分對應」欄位嗎？
...
**問題**：是否需要重新內嵌資料，才能從欄位式拚接切換到圖表式拚接？
...
若要檢查現有的資料集並僅啟用身分服務的資料集，請使用對僅使用`unifiedIdentity`標籤的`/datasets`端點的`PATCH`要求。 例如：
...
**問題**：為什麼拼接的名稱空間值並不總是符合您在CJA連線中其他資料集內可能使用的身分名稱空間值？
