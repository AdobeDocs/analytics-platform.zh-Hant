...
>[!IMPORTANT]
>
>本文說明如何驗證透過Adobe[請求的](/help/stitching/use-stitching.md)資料拼接，並使用新增至重複拼接資料集的拼接身分資料行實作。 已計畫儘快更新此文章，其中包含如何驗證透過Connections UI[啟用](/help/stitching/use-stitching-ui.md)的彙整功能的詳細資料。
...
1. 使用`stitchedID.namespae.code`欄位來建立&#x200B;**[!UICONTROL 電子郵件拼接名稱空間]**&#x200B;維度。 請確定您在元件設定[中指定](/help/data-views/component-settings/include-exclude-values.md)包含排除值，因此您只會考慮您嘗試將資料列提升到的名稱空間值。
