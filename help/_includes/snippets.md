...
此Analytics資料摘要資料行同時包含前置處理版本和後置處理版本（前置詞`post_`）。 前置詞為`post_`的欄包含最終用於報告的值。 下表比較這些資料欄的特性：
...
由於Customer Journey Analytics沒有前置處理和後續處理的概念，在CJA資料摘要中重新建立兩欄相當困難。 如果您想要取得這些欄的近似值，則可以在套用個別歸因設定的情況下使用相同的欄：
