...
| 選項 | 函數 |
|---------|----------|
| **[!UICONTROL 建議]** | 根據其建議對每種類型 (維度、量度、區段和日期範圍) 的元件進行排序。 您或貴組織中的其他人最常使用且最近使用的元件會顯示在每份清單的較高位置。 |
...
## 關聯式模型式 {#relational-model-based}

>[!INFO]
>
>在 Customer Journey Analytics 介面中，「**[!UICONTROL 關聯式]**」資料集可能會標示為「**[!UICONTROL 模型式]**」。
>

## CJA 資料摘要回顧期間 {#cja-df-lookback}

由於 Customer Journey Analytics 對每個元件都使用報告時間歸因，因此其於回顧期間之外沒有持續性概念。 此 Analytics 資料摘要欄會參照延伸至訪客整個歷史記錄的訪客層級行為。 此元件在 Customer Journey Analytics 中的回顧期間越長，就越符合 Adobe Analytics 功能。

## CJA 資料摘要張貼欄 {#cja-df-post}

此 Analytics 資料摘要欄包含預先處理版本和後續處理版本 (前置詞為 `post_`)。 前置詞為 `post_` 的欄包含最終用於報告的值。 多數組織只在 `post_` 欄可用時運用這些欄位。 以下表格呈現這些欄的屬性比較：

| 預先處理的欄值 | 後續處理的欄值 |
| --- | --- |
| 收集時 | 用於報告 |
| 處理規則之前 | 處理規則之後 |
| VISTA 規則之前 | VISTA 規則之後 |
| 未採用配置 | 採用配置 |

由於 Customer Journey Analytics 沒有預先處理和後續處理的概念，因此在 CJA 資料摘要中重新建立兩者欄位相當困難。 如果您希望取得這些欄的近似值，可以將相同的 XDM 欄位拖曳到維度欄中數次，使用不同的歸因設定建立多個元件：

* **經預先處理的欄**：無歸因
* **經後續處理的欄**：套用與資料檢視設定中 Analytics 變數相同的配置及期限設定。 大部分元件會使用「上次」的配置和「造訪」的期限。

## CJA 資料摘要查詢欄 {#cja-df-lookup}

此 Analytics 資料摘要欄使用查詢表格識別所需的值。 在 Customer Journey Analytics 中，值會直接提供，不需要任何查詢表格。 確保您的工作流程可容納最終值，而非中繼查詢值。

## CJA 資料摘要不適用欄 {#cja-df-na}

此 Analytics 資料摘要欄包含因資料架構差異而無法轉譯為 Customer Journey Analytics 對應內容的資料。
