...
>[!NOTE]
>
>您必須擁有 **Select** 套件或更高版本才能使用本節所述的功能。如果您不確定自己擁有哪一種 Customer Journey Analytics 套裝，請聯絡您的管理員。

## Prime 套件 {#prime-package}

>[!NOTE]
>
>您必須擁有 **Prime** 套件或更高版本才能使用本節所述的功能。如果您不確定自己擁有哪一種 Customer Journey Analytics 套裝，請聯絡您的管理員。

## Ultimate 套件 {#ultimate-package}

>[!NOTE]
>
>您必須擁有 **Ultimate** 套件才能使用本節所述的功能。如果您不確定自己擁有哪一種 Customer Journey Analytics 套裝，請聯絡您的管理員。
...
可用的時間比較選項包括前一時段、13 週前、52 週前和自訂日期範圍。如果您選取自訂日期範圍，則會出現其他選項讓您選取數量和顆粒度。如果選取「無」，則會移除日期比較。
...
* **工作階段**：回顧至轉換發生當下此工作階段的起始處。工作階段回顧期間遵循資料釋圖中已修改的[工作階段逾時](/help/data-views/create-dataview.md#session-settings)。選取「**[!UICONTROL 工作階段]**」時，[歸因回顧期間](#atribution-lookback-window)會自動設定為&#x200B;**[!UICONTROL 報告期間]**，且無法變更。
* **人員**：從人員容器的範圍檢視轉換情況。
...
歸因回顧期間是指轉換要納入接觸點時需要回顧的時間量。如果維度項目設定於回顧期間之外，則此值不會納入任何歸因計算。
...
根據您的報告期間 (例如 9 月 10 日至 9 月 24 日)，歸因模型、容器及管道會獲得不同的貢獻度。請參閱下表的範例：

| 模型 | 容器 | 回顧期間 | 說明 |
|---|---|---|---|
| 首次接觸 | 工作階段 | 報告期間 | 歸因只會檢視第三次造訪。在電子郵件和顯示廣告兩者之間，訪客先接觸到電子郵件，因此電子郵件在這次 50 美元的購買動作中獲得 100% 的貢獻度。 |
| 首次接觸 | 個人 | 30 天 | 歸因會檢視所有三次造訪。付費搜尋是第一次接觸，因此在這次 50 美元的購買動作中獲得 100% 貢獻度。 |
| 線性 | 工作階段 | 報告期間 | 貢獻度由電子郵件和顯示廣告平分。兩個管道各獲得 25 美元的貢獻度。 |
| 線性 | 個人 | 30 天 | 貢獻度由付費搜尋、社交媒體、電子郵件和顯示廣告平分。每個管道在這次購買中各獲得 12.50 美元的貢獻度。 |
| J 形 | 個人 | 30 天 | 貢獻度由付費搜尋、社交媒體、電子郵件和顯示廣告平分。<ul><li>60% 歸給顯示廣告，價值 $30 美元，</li><li>20% 歸給付費搜尋，價值 $10 美元，</li><li>剩下的 20% 平分給社交媒體和電子郵件，各為 $5 美元。</li></ul> |
| 時間耗損 | 個人 | 30 天 | <ul><li>顯示廣告接觸點與轉換之間的間隔為零天。`2^(-0/7) = 1`</li><li>電子郵件接觸點與轉換之間的間隔為零天。`2^(-0/7) = 1`</li><li>社交媒體接觸點與轉換之間的間隔為六天。`2^(-6/7) = 0.552`</li><li>付費搜尋接觸點與轉換之間的間隔為九天。`2^(-9/7) = 0.41`</li>將這些值標準化會產生下列結果：<ul><li>顯示廣告：33.8%，價值 $16.88 美元</li><li>電子郵件：33.8%，價值 $16.88 美元</li><li>社交媒體：18.6%，價值 $9.32 美元</li><li>付費搜尋：13.8%，價值 $6.92 美元</li></ul></li></ul> |
...
## 關聯式模型型 {#relational-model-based}

>[!INFO]
>
>在Customer Journey Analytics介面中，**[!UICONTROL 關聯式]**&#x200B;資料集可能標示為&#x200B;**[!UICONTROL 以模型為基礎]**。
>

## CJA資料摘要回顧期間 {#cja-df-lookback}

由於Customer Journey Analytics對每個元件都使用報表時間歸因，因此它沒有回顧期間之外的持續性概念。 此Analytics資料摘要欄會參照可延伸至訪客整個歷程記錄的訪客層級行為。 此元件在Customer Journey Analytics中的回顧期間越長，就越符合Adobe Analytics功能。

## CJA資料摘要張貼欄 {#cja-df-post}

此Analytics資料摘要資料行同時包含前置處理版本和後置處理版本（前置詞`post_`）。 前置詞為`post_`的欄包含最終用於報告的值。 大多陣列織只使用`post_`欄。 下表比較這些資料欄的特性：
...
由於Customer Journey Analytics沒有前置處理和後續處理的概念，在CJA資料摘要中重新建立兩欄相當困難。 如果您想要取得這些欄的近似值，可以將相同的XDM欄位拖曳到維度欄中多次，以使用不同的歸因設定建立多個元件：

* **已預先處理的資料行**：沒有歸因
* **後續處理的資料行**：套用與資料檢視設定中的Analytics變數相同的配置和到期日設定。 大部分元件會使用「上次」的配置和「造訪」的到期時間。

## CJA資料摘要查詢欄 {#cja-df-lookup}

此Analytics資料摘要欄使用查詢表來識別所需的值。 在Customer Journey Analytics中，值會直接提供，不透過任何查詢表格。 確保您的工作流程可容納最終值，而非中繼查詢值。

## CJA資料摘要不適用欄 {#cja-df-na}

由於資料架構差異，此Analytics資料摘要欄包含的資料無法轉譯為Customer Journey Analytics對應專案。

## CJA資料摘要使用者代理 {#cja-df-ua}

您無法同時收集使用者代理程式資訊和裝置查詢資訊；這些維度的母體是互斥的。 在[設定資料流](https://experienceleague.adobe.com/zh-hant/docs/experience-platform/datastreams/configure#geolocation-device-lookup)時，您必須選擇是要直接收集使用者代理程式，還是要收集裝置查閱資訊（根據使用者代理程式）。
