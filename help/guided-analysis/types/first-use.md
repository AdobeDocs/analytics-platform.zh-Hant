...
**[!UICONTROL 首次使用]**&#x200B;檢視會顯示使用者第一次使用產品功能之前和之後所執行的主要指標比較。 此報表的水平軸是事件前後的相對時間間隔，垂直軸則測量所需的索引鍵指標。 圖表中央的垂直長條代表指定使用者首次使用功能時的第0天。 由於使用者不一定會在同一天採用功能，而您的轉出可能會持續數天，因此第0天對於每位使用者來說可能意味著不同的事情。
...
* **[!UICONTROL 檢視]**：在此檢視型別和[版本](release.md)之間切換。
