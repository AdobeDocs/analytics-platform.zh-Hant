...
* **[!UICONTROL 使用者保留率和時數]ñ：** 針對高或低使用者保留率期間提供清楚的視覺效果。 瞭解這些高保留率或低保留率期間，有助於您做出產品決策，以鼓勵高保留率或協助將流失降至最低。
* **[!UICONTROL 行銷活動評估]**：檢視特定行銷活動不僅可協助您瞭解其產生多少流量，也可瞭解行銷活動協助使用者維持參與的程度。
* **[!UICONTROL 使用者生命週期分析]**：分析使用者在整個生命週期中的活躍使用者成長，有助於識別使用者參與度下降的特定階段。 例如，如果個人在入門階段有很高的休眠使用者比例，這可能表示可用性問題或需要更好的產品內指南。
