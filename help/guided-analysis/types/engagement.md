...
標示在此視覺效果圖表最上方的功能，表示這些是參與的使用者中經常使用的功能。 此視覺效果圖表右側標示的功能，表示這些是有效使用者廣泛採用的功能。 功能使用次數的中位數會水平分割圖表。 活躍使用者百分比的中位數會垂直分割圖表。 中位數是根據查詢內選取的事件來計算，而不是根據所有資料來計算。
...
>[!VIDEO](https://video.tv.adobe.com/v/3447478/?captions=chi_hant&quality=12&learn=on)
...
* **改善熱門功能**：較多活躍的使用者但較低使用率的功能，可能表示該功能需求量大但利用率較低。 這些情況讓您有機會從使用者身上進一步了解，哪些改善能讓該功能對他們更有價值。
...
* **[!UICONTROL 計為]**：沿著 x 軸，您可以衡量活躍使用者的每日、每週、每月或每季平均百分比。 Y 軸會根據 x 軸的選擇，自動調整每位使用者的平均次數。
