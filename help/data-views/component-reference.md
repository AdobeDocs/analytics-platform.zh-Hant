...
| [!UICONTROL 帳戶] | 量度 | 根據[!UICONTROL 連線]中指定的帳戶ID。 |
| [!UICONTROL 全域帳戶] | 量度 | 根據[!UICONTROL 連線]中指定的全域帳戶ID。 |
| [!UICONTROL 商機] | 量度 | 機會是根據[!UICONTROL 連線]中指定的機會識別碼。 |
| [!UICONTROL 購買群組] | 量度 | 購買群組，根據[!UICONTROL 連線]中指定的購買群組ID。 |
...
| [!UICONTROL 工作階段開始] | 量度 | 工作階段中第一個事件的事件數。當用於區段定義（例如&#39;[!UICONTROL 工作階段開始] exists&#39;）時，它只會區段到每個工作階段的第一個事件。<p>此元件必須納入您的資料檢視中，以便工作區可使用以下[計算量度](/help/components/calc-metrics/default-calcmetrics.md)： <ul><li>工作階段開始率</li></p> |
| [!UICONTROL 工作階段結束] | 量度 | 工作階段中最後一個事件的事件數。與[!UICONTROL 工作階段開始]類似，它也可用於區段定義中，以區段劃分每個工作階段的最後一個事件。<p>此元件必須納入您的資料檢視中，以便工作區可使用以下[計算量度](/help/components/calc-metrics/default-calcmetrics.md)： <ul><li>工作階段結束率</li></p> |
...
| [!BADGE B2B edition]{type=Informative url="https://experienceleague.adobe.com/en/docs/analytics-platform/using/cja-overview/cja-b2b/cja-b2b-edition" newtab=true tooltip="Customer Journey Analytics B2B edition"}<br/>[!UICONTROL 全域帳戶ID] | 維度 | 當您在連線中使用全域帳戶容器時，[!UICONTROL 全域帳戶ID]。 |
| [!BADGE B2B edition]{type=Informative url="https://experienceleague.adobe.com/en/docs/analytics-platform/using/cja-overview/cja-b2b/cja-b2b-edition" newtab=true tooltip="Customer Journey Analytics B2B edition"}<br/>[!UICONTROL 帳戶ID] | 維度 | [!UICONTROL 帳戶ID]，當您在連線中使用帳戶容器時。 |
| [!BADGE B2B edition]{type=Informative url="https://experienceleague.adobe.com/en/docs/analytics-platform/using/cja-overview/cja-b2b/cja-b2b-edition" newtab=true tooltip="Customer Journey Analytics B2B edition"}<br/>[!UICONTROL 商機ID] | 維度 | 當您在連線中使用機會容器時，[!UICONTROL 機會識別碼]。 |
| [!BADGE B2B edition]{type=Informative url="https://experienceleague.adobe.com/en/docs/analytics-platform/using/cja-overview/cja-b2b/cja-b2b-edition" newtab=true tooltip="Customer Journey Analytics B2B edition"}<br/>[!UICONTROL 購買群組ID] | 維度 | 當您在連線中使用購買群組容器時，[!UICONTROL 購買群組識別碼]。 |
...
| [!UICONTROL 工作階段類型] | 維度 | 此維度有兩個值：1。 [!UICONTROL 第一次]和2。 正在回訪。 [!UICONTROL 首次]條列項目包含已確定為個人定義的首次工作階段之工作階段中所有行為，即此維度的量度。所有其他資料都會包含在[!UICONTROL 回訪]條列項目中 (假設所有資料都屬於一個工作階段)。如果量度不屬於任何工作階段，則將屬於此維度的「不適用」貯體。[了解更多](https://experienceleague.adobe.com/docs/analytics-platform/using/cja-dataviews/data-views-usecases.html#new-repeat) |
