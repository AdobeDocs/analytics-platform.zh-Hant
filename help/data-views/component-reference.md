...
| [!UICONTROL 工作階段開始] | 量度 | 工作階段中第一個事件的事件數。當用於篩選定義時 (例如「[!UICONTROL 工作階段開始]存在」)，它只篩選到每個工作階段的第一個事件。<p>此元件必須包含在資料檢視中，才能進行下列操作 [計算量度](/help/components/calc-metrics/default-calcmetrics.md) 可在Workspace中使用： <ul><li>工作階段開始率</li></p> |
| [!UICONTROL 工作階段結束] | 量度 | 工作階段中最後一個事件的事件數。它類似於[!UICONTROL 工作階段開始]，也可用於篩選定義中，以篩選至每個工作階段的最後一個事件。<p>此元件必須包含在資料檢視中，才能進行下列操作 [計算量度](/help/components/calc-metrics/default-calcmetrics.md) 可在Workspace中使用： <ul><li>工作階段結束率</li></p> |
...
| [!UICONTROL 逗留時間 (秒)] | 量度 | 為維度加總兩個不同值之間的時間。<p>此元件必須包含在資料檢視中，才能進行下列操作 [計算量度](/help/components/calc-metrics/default-calcmetrics.md) 可在Workspace中使用： <ul><li>每人逗留時間</li><li>每工作階段逗留時間</li></p> |
