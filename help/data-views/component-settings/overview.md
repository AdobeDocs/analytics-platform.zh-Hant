...
| [!UICONTROL 結構描述類型] | 不可編輯的欄位，顯示元件的資料類型。雖然您可以在Platform中使用任何支援的結構描述欄位型別，但並非所有欄位型別都在Customer Journey Analytics中受到支援。 以下是支援的資料類型：`Integer`、`Int`、`Long`、`Double`、`Float`、`Number`、`Short`、`Byte`、`String` 和 `Boolean`。 僅限 `String` 目前在查詢資料集中允許使用結構描述資料型別。 |
