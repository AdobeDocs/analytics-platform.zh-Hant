---
title: Customer Journey AnalyticsBI延伸模組
description: 瞭解如何使用Query Service、Power BI、Tableau或其他BI和SQL工具，透過Customer Journey AnalyticsBI擴充功能存取資料檢視。
...
# Customer Journey AnalyticsBI延伸模組
...
* 無需在 BI 工具之內重新建立 Customer Journey Analytics 資料檢視的同等呈現形式。<br/>另請參閱 [資料檢視](data-views.md) 有關資料檢視功能的詳細資訊，以瞭解必須重新建立的內容。
...
* 設定相關產品設定檔、使用者群組及/或個別使用者的功能。 存取需求包括：
   * Adobe Experience Platform查詢服務
   * Customer Journey Analytics中的工作區專案
   * 需要使用的CJA資料檢視
   * 存取資料檢視工具中的BI擴充功能

* 使用不會到期的憑證上的到期，將BI工具連結至 [!DNL Customer Journey Analytics BI extension]. 此 [認證指南](https://experienceleague.adobe.com/en/docs/experience-platform/query/ui/credentials) 提供有關設定即將到期的認證或不即將到期的認證的詳細資訊。
...
1. 在Adobe Experience Platform中查詢並複製您的PostgresSQL認證：
...
1. 若要登入並開始執行您的查詢，請在您的終端機中貼上命令字串。
...
目前， [!DNL Customer Journey Analytics BI extension] 僅支援Power BI和Tableau並進行測試。 其他使用PSQL介面的BI工具也可能會運作，但尚未正式支援。

+++ Power BI

1. 在Adobe Experience Platform中查詢PostgresSQL憑證的詳細資料：
...
1. 在 Power BI 中：
   ...
   1. 在「**[!UICONTROL ** PostgressSQL 資料庫&#x200B;**]**」對話方塊中：
      ...
      1. 提示輸入時 **[!UICONTROL 資料連線能力]** 模式，選取 **[!UICONTROL Directquery]**.

      1. 系統會提示您輸入「**[!UICONTROL 使用者名稱]**」和「**[!UICONTROL 密碼]**」。使用 Experience Platform 查詢[!UICONTROL 認證]的同等參數。


   1. 成功登入後，Customer Journey Analytics資料檢視表會顯示在Power BI的 **[!UICONTROL **&#x200B;導覽器&#x200B;**]**.
...
1. 在 Tableau 中：
   ...
   1. Customer Journey Analytics資料檢視在 **[!UICONTROL **&#x200B;表格&#x200B;**]** 清單。
...
依預設設定，您的資料檢視具有根據其易記名稱產生的表格適用名稱。例如，資料檢視命名為 [!UICONTROL 我的Web資料檢視] 具有檢視名稱 `my_web_data_view`.
...
在 Experience Platform 使用的資料集上所建立的隱私權標籤和原則，可以出現在 Customer Journey Analytics 資料檢視工作流程中。因此，使用 [!DNL Customer Journey Analytics BI extension] 當未遵循所定義的隱私權標籤和原則時，顯示適當的警告或錯誤。
...
* `SELECT COUNT(DISTINCT dimension)` 或 `SELECT APPROX_COUNT_DISTINCT(dimension)` 以計算某個維度的近似不重複值。如需詳細資訊，請參閱 [計算不同的值](#counting-distinct-values).
...
您可以將其他數學套用至中的量度運算式 `SELECT` 而不需在計算量度中定義數學。 下表列出支援的運算式型別。
...
* 若只提供最大值(`timestamp < X` 或 `timestamp <= X`)，範圍是從X減30天到X。
* 如果未提供任何內容，則範圍從現在減去30天到現在。

時間戳記範圍會在RankingRequest中轉換為日期範圍全域篩選器。
時間戳記欄位也可用於日期/時間函式，以剖析或截斷事件時間戳記。

#### 日期範圍

`daterange` 特別欄的運作原理類似於 `timestamp`，但是篩選僅限於全天。`daterange` 也是選用的，而且具有和 `timestamp` 相同的預設範圍。
此 `daterange` 欄位也可用於日期/時間函式，以剖析或截斷事件日期。

此 `daterangeName` 特殊欄可用於使用命名日期範圍篩選查詢，例如 `Last Quarter`.

#### 篩選器ID

`filterId` 特殊欄是選用的，且用於將外部定義的篩選條件套用至查詢。將外部定義的篩選器套用至查詢，類似於將篩選條件拖曳到工作區的面板上。您可以透過 `AND` 的方式提供多個篩選條件 ID。

以及 `filterId`，您可以使用 `filterName` 以使用篩選器的名稱而非ID。

### Where子句

此 `WHERE` 子句的處理步驟有三個：

1. 從以下位置尋找日期範圍： `timestamp`， `daterange`，或 `daterangeName` 特殊欄位。

1. 尋找任何外部定義的 `filterId`s或 `filterName`要包含在篩選中的。

1. 將其餘的運算式轉變為臨時篩選條件。

透過剖析第一層的`AND` (在 `WHERE` 子句中) 來完成處理。每個頂層 `AND`-ed運算式必須符合上述其中一項。 任何比第一層的 `AND` 更深的東西，或者，如果 `WHERE` 子句在頂層使用 `OR`，則作為臨時篩選條件處理。
...
| 函數 | 範例 | 詳細資料 |
|---|---|---|
| [鑄造](https://spark.apache.org/docs/latest/api/sql/index.html#cast) | ``CAST(`timestamp` AS STRING)`` 或 <br/> `` `timestamp`::string `` | 目前不支援類型轉換，但不會引發錯誤。`CAST` 函數被忽略。 |
...
| [結束日期](https://spark.apache.org/docs/latest/api/sql/index.html#to_date) | `` WHERE `timestamp` >= TO_DATE('01/01/2022', 'MM/dd/yyyy') AND `timestamp` < TO_DATE('01/02/2022', 'MM/dd/yyyy') `` | 將日期字串剖析為時間戳記以供 `WHERE` 子句使用，可選擇提供該日期字串的格式。 |
...
| [日期（已截斷）](https://spark.apache.org/docs/latest/api/sql/index.html#date_trunc) | ``SELECT DATE_TRUNC('quarter', `timestamp`)`` | 在傳入的欄位上產生動態的維度識別。<br/>支援的字串詳細程度有：`'YEAR'`、`'Y'`、`'MONTH'`、`'M'`、`'DAYOFMONTH'`、`'DAY'`、`'D'`、`'DAYOFWEEK'`、`'DOW'`、`'DAYOFYEAR'`、`'DOY'`、`'WEEK'`、`'WOY`、`'W'`、`'QUARTER'`、`'QOY'`、`'Q'`、`'HOUR'` 或 `'MINUTE'`。 |
