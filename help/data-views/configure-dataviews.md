...
* 客戶歷程分析中的歸因設定不具破壞性且可回溯性。 換言之，您無法對「客戶歷程分析」中的資料集造成無法彌補的傷害。 即使您不小心刪除了某個項目，您也可以回 [!UICONTROL Experience Platform] 到資料集並重新匯入。 （但請記住，將資料集重新匯入會產生額外成本。）
...
| 設定 | 定義 |
|---|---|
| 工作階段 | 先前稱為「瀏覽」層級。 頁面檢視或作業以外的轉換事件不會與維度或度量產生關聯。 |
