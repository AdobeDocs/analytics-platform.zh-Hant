...
建立資料檢視需要從架構元素建立度量和維度或使用標準元件。 建立量度或維度可為您提供極大的彈性。 以前，假設您在Adobe Experience Platform有資料集，字串欄位是維度，數值欄位是度量。 為了變更這些欄位，您必須在「平台」中編輯您的架構。 資料檢視UI現在可讓量度和維度的定義更自由。

## 設定資料檢視設定和容器

1. 在 Customer Journey Analytics 中，前往&#x200B;**「資料檢視」**&#x200B;標籤。
2. 按一下&#x200B;**添加**&#x200B;以建立新資料視圖並配置其設定。

![](assets/)

| 設定 | 說明／使用案例 |
| --- | --- |
| 連線 | 此欄位會將資料檢視連結至您先前建立的連線，其中包含一或多個Adobe Experience Platform資料集。 |
| 名稱 | 必須為資料檢視命名。 |
| 說明 | 詳細說明並非必要，但建議使用。 |
| 時區 | 選擇您要在哪個時區顯示資料。 |
| 標籤 | 標記可用來將資料檢視整理到不同類別中。 |
| 容器 | 您可以在此處重新命名容器，這是容器在任何以此資料檢視為基礎的工作區專案中的顯示方式。 容器用於篩選和流失／流量，以定義範圍或上下文的寬度或範圍。 [深入了解](https://experienceleague.adobe.com/docs/analytics-platform/using/cja-components/cja-filters/filters-overview.html?lang=en#filter-containers) |
| 人員容器名稱是…… | 人員（預設值）。 「人員」容器包含訪客在指定時間範圍內的每次造訪和頁面檢視。您可以將它重新命名為「使用者」或您偏好的任何其他詞語。 |
| 作業容器名稱是…… | 工作階段（預設值）。 「工作階段」容器可讓您識別特定工作階段的頁面互動、促銷活動或轉換。您可將它重新命名為「造訪」或您偏好的任何其他詞語。 |
| 事件容器名稱是…… | 事件（預設值）。 「事件」容器定義您要在篩選器中納入或排除的頁面事件。 |

接下來，您將從架構元素建立度量和維度。

## 從架構元素建立度量和維度

1. 在[!UICONTROL 客戶歷程分析] > [!UICONTROL 資料檢視]中，按一下[!UICONTROL 元件]標籤。
...
1. 現在，從左側導軌拖曳架構欄位（例如[!UICONTROL pageTitle]）至「量度」或「Dimension」區段。
   ...
   您可以多次將相同的架構欄位拖曳至維度或量度，並以不同的方式設定相同的維度或量度。 例如，從&#x200B;**[!UICONTROL pageTitle]**&#x200B;欄位，您可以建立名為「產品頁面」的維度，以及另一個「錯誤頁面」等。 從&#x200B;**[!UICONTROL pageTitle]**;欄位中，您也可以從字串值建立量度。 例如，您可以建立一或多個具有不同歸因設定且不同包含／排除值的&#x200B;**[!UICONTROL 訂購]**&#x200B;量度。
...
| 路徑 | 必填.一個不可編輯的欄位，顯示元件來自的架構路徑。 |
| 隱藏報表中的元件 | 預設= off。 可讓您在報告中使用元件時，將元件組織出「資料檢視」。 這不會影響權限，只影響元件組織。 換言之，您可以在報告中隱藏元件，使其不受非管理員的影響。 管理員仍可以按一下Analysis Workspace專案中的「顯示所有元件」來存取它。 |
...
重要：當您將此欄位變更為自訂值時，自訂值會被視為合法字串值。 因此，如果在此欄位中輸入&quot;Red&quot;值，資料中出現的字串&quot;Red&quot;例項也會在您所指定的相同行項目下滾動。

另請注意，您在此欄位中指定的任何項目，都可用於報告中「無值」行項目的特殊UI處理，如「無值選項」設定所述。 （不確定這意味著什麼。）

![](assets/no-value-options.png)

| 設定 | 說明／使用案例 |
| --- | --- |
| 如果顯示，請調用[!UICONTROL 無值]... | 您可以在此處將&#x200B;**[!UICONTROL No value]**&#x200B;更名為其他值。 |
| 預設不顯示&#x200B;**[!UICONTROL 無值]** | 不會在報表中顯示此值。 |
| 預設顯示&#x200B;**[!UICONTROL 無值]** | 確實會在報表中顯示此值。 |
| 將&#x200B;**[!UICONTROL 無值]**&#x200B;視為值 | 例如，如果您以行動裝置類型為維度，您可將&#x200B;**[!UICONTROL No value]**&#x200B;項目重新命名為「Desktop」。 |
...
| --- | --- |
| 設定持續性 | 切換鍵 |
| 配置 | 可讓您指定用於維度的永續性分配模型。 選項包括：最近，原稿，例項，全部。 如果您想要保留值（類似於傳統Analytics中的eVar），您會在這裡設定值。 唯一的關鍵區別是，您可以設定的最大持久性為90天。 此外，[!UICONTROL Never expire]不是選項。 |
| 過期 | 可讓您指定維度的永續性視窗。 選項包括：工作階段（預設值）、人員、時間、量度。您可能需要能夠使購買的維度過期（例如內部搜尋詞或其他銷售使用案例）。 「量度」可讓您指定任何已定義量度作為此維度的有效期（例如「購買」量度）。 |
...
| 設定 | 說明／使用案例 |
| --- | --- |
| 貯體值 | 可讓您建立數值維度的分區版本。 這可讓您報告收入區間或其他數值作為報告中的維度。 最多可以建立5個儲存桶。 |
| 最多 | 可讓您指定第一個數值維度儲存段的邊界。 這僅適用於數值維度。 |
| 介於和最多 | 可讓您指定後續數值維度區間的邊界。 |
| 新增貯體 | 可讓您新增另一個儲存貯體至數值維度分區。 |

## 使用標準元件

除了從結構元素建立量度和維度外，您也可以在資料檢視中使用標準元件。

標準元件是不是從資料集架構欄位產生，而是由系統產生的元件。 在任何「資料檢視」中都需要一些系統元件，以利在Analysis Workspace建立報告功能，而其他系統元件則是選用的。
...
| --- | --- | --- |
| 人物 | 量度 | 在傳統Analytics中，先前稱為[!UICONTROL 獨特訪客]。 此量度以連線中指定的人員ID為基礎。 |
| 工作階段 | 量度 | 在傳統Analytics中，先前稱為[!UICONTROL 瀏覽]。 此量度以下指定的作業化設定為基礎。 |
| 事件 | 量度 | 在傳統Analytics中，先前稱為[!UICONTROL Occurrences]。 此度量表示Connection中所有事件資料集的行數。 |
| 日 | 維度 |  |
| 週 | 維度 |  |
| 月 | 維度 |  |
| 季 | 維度 |  |
