...
* 簡單操作，例如[小寫](./derived-fields.md#lowercase)、[Trim](./derived-fields.md#trim)或[Case When](./derived-fields.md#case-when)檢查頁面URL通常比低基數欄位的相同邏輯更昂貴。
...
* [Regex Replace](./derived-fields.md#regex-replace)或以Regex為基礎的條件使用廣泛的模式；較簡單的[Case When](./derived-fields.md#case-when)函式具有&#x200B;**[!UICONTROL Contains]**&#x200B;或&#x200B;**[!UICONTROL Starts with]**&#x200B;是較好的替代方案。
...
* 保留[下一個或上一個](./derived-fields.md#next-or-previous)，用於進階多步驟路徑或funnel標籤，其維度持續性無法單獨達成（例如：頻道序列串連）。
...
>[!NOTE]
>
>儘可能減少對複雜衍生欄位中高基數欄位的依賴（例如：使用標準化索引鍵或群組分類），以限制查詢成本及達到[運運算元或函式限制](./derived-fields.md#limitations)的可能性。

Customer Journey Analytics [檔案](./derived-fields.md#limitations)每個衍生欄位的最大函式和運運算元，包括每個函式型別的限制。atterns**
