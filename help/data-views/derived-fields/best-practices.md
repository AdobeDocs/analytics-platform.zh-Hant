...
* 歸因和持續性：避免在具有[下一個或上一個](./derived-fields.md#next-or-previous)或其他循序邏輯的衍生欄位中模擬維度。 使用維度資料檢視[歸因](/help/data-views/component-settings/attribution.md)和[持續性](/help/data-views/component-settings/persistence.md)設定。
...
* 如果輸出是標籤：
   * 將元件型別設定為&#x200B;**[!UICONTROL Dimension]**，並相應地設定[歸因](/help/data-views/component-settings/attribution.md)和[持續性](/help/data-views/component-settings/persistence.md)設定。
...
* 具有歸因或持續性的備援：資料檢視歸因設定可更好地涵蓋某些使用案例（例如工作階段上的上次接觸管道歸因）。

### 推薦

* 對於類似標準歸因的模式，在資料檢視中使用維度[歸因](/help/data-views/component-settings/attribution.md)和[持續性](/help/data-views/component-settings/persistence.md)設定，而不是使用[下一個或上一個](./derived-fields.md#next-or-previous)來模擬。
* 保留[下一個或上一個](./derived-fields.md#next-or-previous)，以取得進階多步驟路徑或funnel標籤，這些標籤無法單獨實現歸因（例如：頻道序列串連）。
...
* 語意不符：維度的標籤建議的[歸因](/help/data-views/component-settings/attribution.md)或[到期](/help/data-views/component-settings/persistence.md)行為與實際設定不同（例如：首次接觸或原始來源）。
* 這種不相符會增加分析師誤解報表或比較名稱相似但使用不同歸因模型的元件的風險。
