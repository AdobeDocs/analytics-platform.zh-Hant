...
| 刪除 GDPR | 請注意，現在與 [!UICONTROL Experience Platform] 協調處理 GDPR - CJA 會繼承 [!UICONTROL Experience Platform] 對基礎資料集所做的所有資料變更。 |
...
| AAM 對象 | 如果客戶有使用 [!UICONTROL Analytics Data Connector] 資料集，此資料將成為 ADC 資料的一部分。 |
...
| 跨裝置身分識別拼接 | 客戶受限於只能透過 Query Service 對資料進行「一次性」拼接，或目前必須在 [!UICONTROL Experience Platform] 資料內嵌前將此邏輯套用至資料。 |
...
| Ad Hoc Analysis |  |
| Data Warehouse報表 | [!UICONTROL Experience Platform Query Service] 將是這些使用案例在 CJA 中的新介面。 |
| Mobile Services |  |
| 資料饋送 |  |
