...
| 異常偵測 | 完全支援 |
| 歸因 IQ | 完全支援 |
| 計算量度 | 全力支援；請注意，傳統Analysis Workspace中的任何現有計算量度都不會移植至CJA。 |
| 跨裝置/跨管道彙整 | 全力支援；請參閱[跨管道分析](/help/connections/cca/overview.md)。 |
| 日期比較 | 完全支援 |
| 維度 | 全力支援；CJA採用XDM並支援不限數量的維度。 CJA未與傳統Adobe Analytics的自訂eVar或prop系結。 |
...
| 刪除 GDPR | 全力支援；請注意，現在已與[!UICONTROL Adobe Experience Platform]協調處理GDPR。 CJA會繼承[!UICONTROL Experience Platform]對基礎資料集所做的所有資料變更。 |
| 清單變數/清單 Prop | 全力支援；CJA採用XDM並支援不限數量的字串陣列，其使用方式與listVars類似。 |
| 量度 | 全力支援；CJA採用Experience Data Model(XDM)並支援不限數量的量度，且不會與傳統Analytics的自訂成功事件系結。 請注意，傳統 Analytics 的部分標準量度已重新命名：訪客 = 人員，造訪 = 工作階段，點擊 = 事件。 |
| PDF 匯出 | 完全支援 |
| 專案策劃 | 完全支援 |
| 專案連結 | 完全支援 |
| 報表時間處理功能 | 全力支援；CJA僅依賴「報表時間處理」功能。 |
| 報表 API 存取 | 全力支援；可透過[CJA API](https://www.adobe.io/cja-apis/docs/)取得。 |
| 排程報表/專案 | 完全支援 |
| 區段 | 全力支援；現在稱為「篩選器」 — 請注意，傳統Analysis Workspace中的任何現有區段都不會移植至CJA。 |
| 使用者權限/資料存取控制 | 全力支援；CJA可區分[Adobe Admin Console](https://experienceleague.adobe.com/docs/core-services/interface/administration/admin-getting-started.html?lang=en)產品管理員和使用者。 只有產品管理員才能 <ul><li>建立/更新/刪除連線或資料檢視</li><li>更新/刪除其他使用者建立的專案、篩選器或計算量度，以及</li><li>和所有使用者共用 Workspace 專案.</li></ul> |
| 虛擬報表套裝 | 全力支援；現在稱為[資料檢視](/help/data-views/create-dataview.md)。 |
| VRS 元件策劃 | 全力支援；現在是資料檢視的一部分。 |
...
| 客戶屬性 | 現在稱為「設定檔資料集」，系統不會自動從Experience Cloud匯入這些資料集，但必須先上傳至AEP，才能在CJA中使用。 |
| 日期範圍 | 除了已規劃的自訂日曆支援外，支援所有日期範圍功能。 |
...
| 地域劃分維度 | 所有收集到Adobe Analytics的「地域劃分/地理位置」都會透過Analytics Data Connector流入CJA。 未使用Analytics Data Connector的實作（例如仰賴AEP Web SDK進行數位資料收集的實作）將無法自動執行完整的地理查閱（支援國家/地區和州、城市和郵遞區號）。 |
...
| 處理規則 | 對於以 Analytics Data Connector 為基礎的資料集，仍會套用處理規則。[Adobe Experience Platform中的資料準](https://experienceleague.adobe.com/docs/experience-platform/data-prep/home.html?lang=en) 備功能也可作為直接前往Platform之資料的處理規則的取代。 |
