...
| 報告 API 存取權 | 全力支援；可透過[CJA API](https://www.adobe.io/cja-apis/docs/)取得。 |
| 排程報告/專案 | 完整支援 |
| 區段 | 全力支援；現在稱為「篩選器」 — 請注意，傳統Analysis Workspace中的任何現有區段都不會移植至CJA。 |
| 使用者權限/資料存取控制 | 全力支援；CJA可區分[Adobe Admin Console](https://experienceleague.adobe.com/docs/core-services/interface/administration/admin-getting-started.html?lang=zh-Hant)產品管理員和使用者。 只有產品管理員可以 <ul><li>建立/更新/刪除連線或資料檢視</li><li>更新/刪除其他使用者建立的專案、篩選或計算量度，以及</li><li>與所有使用者共用 Workspace 專案。</li></ul> |
