...
| 任務 | 詳細資料 |
| --- | --- |
| **步驟1：如果您要從Adobe Analytics升級至Customer Journey Analytics：選擇升級路徑並將資料傳送至Adobe Experience Platform** | 從Adobe Analytics升級為Customer Journey Analytics時，有各種可用的路徑。 每個可能的升級路徑都有其各自的優缺點，而適合一個組織的路徑可能對另一個組織而言並無意義。 <p>若要開始從Adobe Analytics升級至Customer Journey Analytics，請參閱 [開始升級至Customer Journey Analytics](/help/getting-started/cja-upgrade/cja-upgrade-getstarted.md). <!-- [Utilizing Adobe Analytics report suite data in Customer Journey Analytics](/help/getting-started/aa-vs-cja/aa-data-in-cja.md) --> </p> |
