...
| **可用 Experience Cloud ID (ECID)** | 否 | 無 | 是 | 是 | 無 | 否 |
| **可用時間戳記** | 否 | 否 | 無 | 是 | 無 | 否 |
| **處理層級** | 完整處理 | 完整處理 | 完整處理 | 完整處理 | 完整處理 | 完整處理 |
| **包含機器人篩選資料** | 否 | 否 | 否 | 否 | 否 | 否 |
| **低流量 (超出不重複值) 出現** <br> [了解更多](/help/components/dimensions/high-cardinality.md) | 是 | 是 | 無 | 無 | 是 | 是 |
| **可見列限制 (分頁前)** | 400 | 50,000 | 無限制 | 無限制 | 50,000 | 50,000 |
| **多個資料檢視** | 是 | 是 | 無 | 無 | 是 | 是 |
...
| **計算量度** <br> [了解更多](/help/components/calc-metrics/calc-metr-overview.md) | 是，且具有[歸因](/help/analysis-workspace/attribution/overview.md) | 是，使用 Attribution | 否 | 否 | 是，使用 Attribution | 是，使用 Attribution |
