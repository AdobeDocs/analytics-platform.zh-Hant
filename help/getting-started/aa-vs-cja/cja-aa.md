...
| 串流 Media Analytics | 媒體資料將會在 2022 年 7 月 30 日當作工作區中的「媒體同時檢閱者」面板和「媒體播放時間」面板的一部分提供。 |
| 使用者權限/資料存取控制 | 完整支援；CJA 會區分 [Adobe Admin Console](https://experienceleague.adobe.com/docs/core-services/interface/administration/admin-getting-started.html) 產品管理員與使用者。 只有產品管理員可以 <ul><li>建立/更新/刪除連線或資料檢視</li><li>更新/刪除其他使用者建立的專案、篩選或計算量度，以及</li><li>與所有使用者共用「工作區」專案。</li></ul> |
...
| 功能 | 附註 |
| --- | --- |
| A4T | 透過 [Adobe Analytics 來源連接器](https://experienceleague.adobe.com/docs/experience-platform/sources/connectors/adobe-applications/analytics.html)中的欄位提供支援。 |
