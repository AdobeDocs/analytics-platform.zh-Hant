...
| 清單變數/清單 Prop | 完整支援。Customer Journey Analytics 使用 XDM 並支援無限制字串陣列，其使用方式與 listVars 類似。 |
...
| 專案策劃 | 完整支援 |
| 專案連結 | 完整支援 |
| 報告時間處理 | 完整支援；Customer Journey Analytics 完全依靠報告時間處理。 |
...
| 自訂工作階段化 | 自訂工作階段化可透過 [工作階段設定](../../data-views/create-dataview.md#session-settings) 在資料檢視中。 另請參閱  [內容感知作業](../../data-views/context-aware-sessions.md) 以取得詳細資訊。 <br/>透過Adobe Experience Platform Mobile SDK支援行動背景事件的處理。 另請參閱 [邊緣網路的生命週期](https://developer.adobe.com/client-sdks/documentation/lifecycle-for-edge-network/) 以取得詳細資訊。 |
...
| 區段 IQ | 已規劃提供支援。 |
| 交易 ID 資料來源 | 已規劃提供支援。 |
| 將專案/篩選器/計算量度從 Adobe Analytics 移至 Customer Journey Analytics | 已規劃提供支援。 |
| 摘要層級資料來源 | 已規劃提供支援。 |
...
| 無限制基數/唯一值 | Customer Journey Analytics 支援可在單一維度中報告的無限制唯一值或維度項目。Adobe Analytics 最多 500,000 個唯一值。無限制的唯一值或維度消除了大規模分析實施中目前存在的報告和分析限制。 |
| 報告時間轉換 | Customer Journey Analytics 中的報告時間轉換 (通常稱為資料檢視) 可讓您進一步解釋來自連線的資料。您可以變更或刪除資料而無需重新實施；使用子字串來操作維度；從任意值建立量度；篩選子事件。而且轉換都是以非破壞性執行的。Adobe Analytics 透過虛擬報告套裝和工作階段化提供有限的功能。 |
| 實驗分析 | 對於任何定義為連線一部分之資料來源的實驗，Customer Journey Analytics 都可以評估其提升度和可信度。透過此評估，您可以了解任意管道中客戶互動之間的因果關係。Analytics 只能透過 Analytics for Target (A4T) 整合進行實驗分析。 |
...
| 增強型安全性和隱私權選項 - HIPAA 準備程度 | Customer Journey Analytics 符合 HIPAA 標準，並提供額外的安全性選項以實現法規合規性。Adobe Analytics 尚未符合 HIPAA 標準。 |
