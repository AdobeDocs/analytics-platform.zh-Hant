...
| 自訂工作階段化 | 自訂作業化可以透過資料檢視中的 [工作階段設定](../../data-views/create-dataview.md#session-settings) 來設定。如需詳細資訊，請參閱[工作階段設定](../../data-views/session-settings.md)。<br/>使用 Adobe Experience Platform Mobile SDK 支援，處理行動背景事件。更多資訊，請參閱[邊緣網路的生命週期](https://developer.adobe.com/client-sdks/documentation/lifecycle-for-edge-network/)。 |
