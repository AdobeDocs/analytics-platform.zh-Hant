...
| 異常偵測 | 完整支援 |
| Attribution IQ | 完整支援 |
| 對象發佈 | 完整支援。 在 AA 中稱為「區段發佈」(將區段從工作區傳送到 Experience Cloud)。 [對象發佈](/help/components/audiences/audiences-overview.md)會將對象傳送到 Experience Platform 中的即時客戶個人檔案。 |
...
| 串流 Media Analytics | 媒體資料是 [Adobe Analytics 來源連接器](https://experienceleague.adobe.com/docs/experience-platform/sources/connectors/adobe-applications/analytics.html?lang=zh-Hant)的一部分。 |
