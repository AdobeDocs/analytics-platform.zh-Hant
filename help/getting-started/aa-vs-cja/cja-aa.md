...
| 串流媒體 Analytics | 媒體資料可使用分析來源聯結器當作工作區中的「媒體同時檢閱者」面板和「媒體播放時間」面板的一部分提供。 |
...
| IP 模糊化 | 對於使用Analytics來源聯結器將來自Adobe Analytics的資料填入到Customer Journey Analytics中的Customer Journey Analytics客戶：在Adobe Analytics中套用的IP模糊化設定會流向Customer Journey Analytics資料。 您可以視需要在 Adobe Analytics 中控制這些設定。<p>適用於使用Experience PlatformWeb SDK將資料填入平台並直接Customer Journey Analytics的Customer Journey Analytics客戶。 您可以在Platform中使用資料收集的資料準備，以設定根據您公司的需求模糊化IP位址的規則。 |
...
| 處理規則、VISTA 規則、行銷管道處理規則 | 透過WebSDK資料集和Analytics來源聯結器的資料，都支援使用Adobe Experience Platform資料準備功能。 |
...
| 機器人篩選 | 對象 [Analytics來源聯結器](https://experienceleague.adobe.com/docs/experience-platform/sources/connectors/adobe-applications/analytics.html?lang=zh-Hant)以資料集為基礎，系統會套用機器人篩選。 其他資料集的一般機器人篩選邏輯不會由執行 [!UICONTROL Experience Platform] 或Customer Journey Analytics。 |
| 裝置、瀏覽器、推薦者、技術維度 | 支援 [Analytics來源聯結器](https://experienceleague.adobe.com/docs/experience-platform/sources/connectors/adobe-applications/analytics.html?lang=zh-Hant)以資料集為基礎。 請參閱 [透過ADC支援哪些Analytics變數的檔案](https://experienceleague.adobe.com/docs/experience-platform/sources/connectors/adobe-applications/mapping/analytics.html?lang=zh-Hant).<p>如果您使用Experience PlatformWeb SDK資料彙集，目前不支援根據裝置查詢的裝置和維度。 已規劃於未來支援。 |
...
| A4T | 透過「 」中的欄位提供部分支援 [Analytics來源聯結器](https://experienceleague.adobe.com/docs/experience-platform/sources/connectors/adobe-applications/analytics.html?lang=zh-Hant). 已規劃目標活動和體驗支援 A4T 易記名稱。 |
