---
title: 比較Adobe Analytics與Customer Journey Analytics報告功能的資料處理
...
相較之下，Customer Journey Analytics的設計目的是在整理及儲存資料之前，只需最少的前期收集時間處理。 Customer Journey Analytics的底層架構設計為可在報告時間處理儲存的資料，並提供其強大的報告時間處理功能，不僅適用於Workspace，更重要的是，可透過 [元件](/help/data-views/component-settings/overview.md) 和 [衍生欄位](/help/data-views/derived-fields/derived-fields.md) 在您的資料檢視中。

![Customer Journey Analytics報表時間處理](../assets/cja-processing.png)

了解各種報告功能的資料處理差異，有助於了解哪些量度可用於哪裡，以及其不同的原因。

例如，由於Adobe Analytics中的「造訪」量度是在資料處理時定義的，而Customer Journey Analytics中的「工作階段」是在報告時計算的，因此這兩個量度可能會因Customer Journey Analytics資料檢視中用於工作階段定義的規則而異。

此外，在Analytics來源聯結器建立的資料集中，造訪和工作階段都不能作為量度，因此需要您在查詢邏輯中定義工作階段才能進行比較。

## 術語 {#terms}

下表定義套用至Adobe Analytics和Customer Journey Analytics之處理邏輯型別的術語：
...
| Adobe Analytics [直播串流](https://github.com/AdobeDocs/analytics-1.4-apis/blob/master/docs/live-stream-api/getting_started.md) | <ul><li> 處理規則</li><li>VISTA 規則</li><ul> |   | <ul><li>點擊層級行銷頻道規則</li><li>造訪層級行銷頻道規則</li><li>造訪邏輯</li><li>歸因邏輯</li><li>區段邏輯</li><li>計算量度</li><li>跨裝置分析</li></ul> |  |
