...
| 機會（選擇性） | 一起銷售的產品和服務集合。 在銷售週期中，銷售機會通常涉及不同的階段，直到銷售結束為止。<br>您應該使用資料來測量透過銷售funnel的商機進展。 例如，提供從階段3移至階段4之熱門商機詳細資訊的報表。 |
...
| 需求 | **時間戳記**<br>&#x200B;每個記錄都需要：<ul><li>帳戶 ID</li><li>全域帳戶ID （選擇性）</li></ul> | **帳戶ID**<br>&#x200B;記錄需要容器ID，例如：<ul><li>帳戶</li><li>人員</li><li>機會</li><li>購買群組</li></ul> | **相符的索引鍵**<br>&#x200B;記錄需要包含在容器或事件資料集中的識別碼，例如：<ul><li>行銷活動 ID</li><li>內容 ID</li><li>產品 ID</li></ul> |
...
與Customer Journey Analytics類似，事件型時間序列資料是Customer Journey Analytics B2B edition的核心。 帳戶式連線的主要差異在於，您事件資料集中的每個記錄都需要一個帳戶ID，而不是人員ID。
