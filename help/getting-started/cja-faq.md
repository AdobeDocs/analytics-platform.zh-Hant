...
| 非縫合配置檔案資料集記錄的預期行為是什麼？ | **示例方案**:通過使用 `CRMid` 作為人員ID。 一個是Web事件資料集 `CRMid` 記錄中。 另一個資料集是CRM配置檔案資料集。 40%的CRM資料集 `CRMid` 在Web事件資料集中。 Web事件資料集中沒有其他60%的記錄 — 這些記錄是否在Analysis Workspace的報告中顯示？<p> **答案**:沒有與其關聯的事件的配置檔案行儲存在CJA中。 但是，在出現與該ID相關的事件之前，您不能在Analysis Workspace查看它們。 |
