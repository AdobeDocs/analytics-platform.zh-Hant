...
Customer Journey Analytics可讓您控制如何在Analysis Workspace中連線任何常見客戶ID的線上和離線資料，讓您可對客戶資料執行歸因、區段、流量、流失等分析。
...
您現在可以在Analysis Workspace中劃分面板。 在某些情況下，建議先對面板中的資料和視覺效果進行劃分，再對面板套用區段。

[![影像](assets/learn-more-button.svg)](/help/analysis-workspace/c-panels/panels.md#break-down-a-panel)

>[!TAB 多個維度資料行]

您最多可以在自由表格中包含5個維度欄，讓您並排檢視多個維度專案。 每一列維度項目的行為都像是單一串連的維度項目。

[![影像](assets/learn-more-button.svg)](/help/analysis-workspace/visualizations/freeform-table/freeform-table-multidimensions.md)
...
Customer Journey Analytics B2B Edition 透過提供可操作的客戶洞察來推動收入成長，幫助 B2B 公司協調其行銷、銷售和產品團隊。 由於客戶是資料模型的中心，因此所有分析都集中在客戶歷程上。
