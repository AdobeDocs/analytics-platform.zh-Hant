...
description: 了解從 Adobe Analytics 升級至 Customer Journey Analytics 時的建議步驟
role: Admin
...
從 Adobe Analytics 升級到 Customer Journey Analytics 的推薦流程是 Experience Platform Web SDK 的新實施，這是最適月合 Customer Journey Analytics 的資料收集方法。 若結合 Web SDK，Adobe 也建議使用 Analytics 來源連接器來幫助您轉變為 Customer Journey Analytics。 使用 Analytics 來源連接器保留歷史 Adobe Analytics 資料並執行並排資料比較。

在您使用 Experience Platform Web SDK 獲得足夠的歷史資料並完全轉換至 Customer Journey Analytics 後，即可關閉 Analytics 來源連接器並單獨使用 Web SDK。

>[!NOTE]
>
>如果本節所述的升級步驟不適用於您的組織，請使用 Customer Journey Analytics 升級指南，以動態方式產生適合您組織獨特情況的升級步驟。 (若要自 Customer Journey Analytics 存取指南，請選取「**[!UICONTROL 工作區]**」索引標籤，然後在左側面板中選取「**[!UICONTROL 升級至 Customer Journey Analytics]**」。 按照畫面上的指示進行操作。)
...
1. **設定 Adob&#x200B;&#x200B;e Analytics 來源連接器 (可轉變歷史資料)**
   ...
   為了協助順利轉變為搭配 Customer Journey Analytics 使用 Experience Platform Web SDK，Adobe 也建議使用 Adobe Analytics 來源連接器。 這樣可以保留歷史資料，並在 Customer Journey Analytics 中檢視來自現有 Adobe Analytics 實施的資料，與來自新的 Experience Platform Web SDK 實施的資料並排比較。

   Analytics 來源連接器可讓您：

   * 將歷史 Adobe Analytics 報告套裝資料匯入至 Adobe Experience Platform 和 Customer Journey Analytics。

     只要您需要保留歷史 Adobe Analytics 資料，您就可以讓 Analytics 來源連接器持續運作。

   * 在 Customer Journey Analytics 中檢視透過原始 Adobe Analytics 實施 (AppMeasurement、Analytics 擴充功能或 Web SDK 擴充功能) 收集的資料。 您可以將此資料與新 Web SDK 實施的資料並排比較。
...
1. (選用) 如果您在 Adobe Analytics 中使用分類資料，則可以將分類資料新增至 Customer Journey Analytics 中的資料集。
...
1. (選用) 如果您想將 Customer Journey Analytics 與 Adobe Journey Optimizer 整合，請在實施中使用個人化物件，以便在 Adobe Journey Optimizer 中使用。
...
1. [將舊實作中的Adobe Analytics資料與新實作中的Customer Journey Analytics資料進行比較](https://experienceleague.adobe.com/zh-hant/docs/cx-enterprise-ai/experience-cloud-ai/coworker/chat/use-cases/data-insights/data-validation-aa-cja)，確定您瞭解差異及差異存在的原因。
...
1. 了解關於 [Customer Journey Analytics 中的功能支援](/help/getting-started/aa-vs-cja/cja-aa.md){target="_blank"}。 Customer Journey Analytics 支援大多數 Adobe Analytics 功能，Customer Journey Analytics 還提供了許多附加功能。

1. 當您的 Customer Journey Analytics Web SDK 實施完成並且您對所收集的資料感到滿意時，可停用 Adobe Analytics。

   Adobe 建議您在實施 Customer Journey Analytics 後，讓 Adobe Analytics 環境持續執行一段時間。
