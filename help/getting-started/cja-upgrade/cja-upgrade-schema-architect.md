...
| 圖層 | 主要用途 | 屬於什麼 | 不屬於 |
|---|---|---|---|
| **XDM結構描述** | 定義所收集資料的持久結構和意義 | 事件和實體形狀、欄位含義、關係、允許值、跨管道重複使用 | 編號的「槽」(eVar1/prop1)、歸因/持續性邏輯、報表專用因應措施 |
| **資料檢視** | 定義收集到的資料在分析中的行為方式 | 元件設定、歸因和持續性行為、衍生欄位、篩選量度、計算量度 | 欄位的基本意義；該意義在結構描述中應該穩定 |
...
| Adobe Analytics的常見習慣 | XDM + CJA中更好的方法 |
|---|---|
...
| 設計變數以「預先解決」報表 | 設計您的結構描述以可靠地擷取事實，並使用資料檢視解決報告語意 |
...
此模式支援單一組織結構描述策略，不會強制每個團隊填入不適用於其頻道的欄位。
...
在Adobe Analytics中，許多團隊會將`events`變數視為量度的去向。 在Customer Journey Analytics中，您可以根據您需要計算的專案及您想要解譯的方式，以多種方式模型化量度。
...
>[!TIP]
>
>在決定某個專案是否應是專用欄位與稍後是衍生欄位時，如果結構描述中有廣泛用途且穩定，則偏好擷取持久事實。 您可以在收集資料後，使用衍生欄位來更正或改變資料的外觀。

## 在轉換期間維持與Adobe Analytics的同等功能，避免方案拖累
