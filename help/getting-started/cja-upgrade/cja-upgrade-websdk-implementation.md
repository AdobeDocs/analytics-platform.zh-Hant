...
description: 了解升級至 Customer Journey Analytics 時的 Web SDK 實施選項
role: Admin
...
>title="Web SDK JavaScript 程式庫 (alloy.js)"
>abstract="在您網站的每個頁面上加入 Web SDK 程式庫 (alloy.js)。"
...
>title="為指定的屬性實施 Web SDK"
>abstract="選取升級指南中所需的實施類型，以了解更詳細的說明。"
...
>abstract="與標記管理系統的管理員合作，將 Web SDK 程式庫新增至您的網站。<br><br>此任務的完成時間有很大程度取決於負責標記管理系統之個人的回應能力。 新增 Web SDK 程式庫可能會與相關聯的實施邏輯整合在一起，並在組織的標準發行週期內進行部署。"
...
使用 Adobe Experience Platform Web SDK 的三種支援方式：

* [Web SDK 標記擴充功能](https://experienceleague.adobe.com/zh-hant/docs/experience-platform/web-sdk/install/extension)：Adobe 建議使用此方法。 在您的網站上安裝標記載入器，然後使用 Adobe Experience Platform 資料彙集 UI 來設定您的實施。

* [Web SDK JavaScript 資料庫](https://experienceleague.adobe.com/zh-hant/docs/experience-platform/web-sdk/install/library)：引用 CDN 託管的資料庫檔案，或使用您自己的基礎架構來託管資料庫檔案。 在您網站的程式碼內呼叫該程式庫。
