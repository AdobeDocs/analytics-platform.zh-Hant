...
在比較 Real-time CDP 與 Customer Journey Analytics 之間的客群會籍數時，一定要牢記這兩個工具有不同的用途。 Real-time CDP 使用客戶輪廓資料來針對個別消費者提供數位體驗，而 Customer Journey Analytics 的設計目的則是幫助使用者了解關鍵商業量度和區段的模式。 雖然從 Customer Journey Analytics 到 Real-time CDP 的客群發佈可讓這些工具的使用者利用在 Customer Journey Analytics 中獲得的知識，以原生方式輕鬆地「啟用」洞察，但這些工具還是有截然不同的用途。
...
Customer Journey Analytics 可以設定為使用[拼接](../stitching/overview.md)。 若使用[欄位式拼接](/help/stitching/fbs.md)做為拼接機制，您可以從資料湖中的資料集指定一個識別碼來拼接此資料集中的資料，其目的是透過已獲改善的合併輪廓來提升資料集。 若您使用[圖表式拼接](/help/stitching/gbs.md)做為拼接機制，則類似的過程是根據指定的身分識別命名空間使用身分識別圖。
