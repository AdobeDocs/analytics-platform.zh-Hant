...
| 功能 | 說明 | [全面發佈](https://experienceleague.adobe.com/docs/analytics/technotes/releases.html) - 目標日期 |
| ----------- | ---------- | ----- |
| [!UICONTROL 連線]資料保留的滾動時段 | **注意：請聯絡客戶服務或您的Adobe客戶團隊，以實作此設定。 尚未透過Customer Journey Analytics UI提供。**<p>可讓您將Customer Journey Analytics資料保留設定定義為以月為單位的滾動時段（3個月、6個月等），位於 [!UICONTROL 連線] 層級(非於 [!UICONTROL 資料集] 層級)。 資料保留是以事件資料集時間戳記為基礎，僅適用於事件資料集。由於無適用的時間戳記，因此基本資料或查詢資料集不存在資料保留設定。主要優點在於您只會儲存或報告適用且實用的資料，並刪除不再實用的舊資料。這有助於您未超過合約限制，並減少超額使用費用的風險。 | 2021 年 10 月 7 日 |
...
| 報告的日光節約時間支援 | Customer Journey Analytics中的所有資料都會以UTC儲存（而不是以特定時區儲存）。 [!UICONTROL 資料檢視] Customer Journey Analytics現在可以根據日光節約時間來轉換資料，以代表1小時的增減量。 [了解更多](https://experienceleague.adobe.com/docs/analytics-platform/using/cja-dataviews/create-dataview.html?lang=zh-Hant#calendar) | 2021 年 9 月 16 日 |
...
| 功能 | [全面發佈](https://experienceleague.adobe.com/docs/analytics/landing/an-releases.html) - 目標日期 | 說明 |
| ----------- | ---------- | ----- |
| 對 [!UICONTROL Adobe Analytics 儀表板]的支援 | 2021 年 3 月 25 日 | [!UICONTROL Customer Journey Analytics] 現在支援 [!UICONTROL Adobe Analytics儀表板計分卡產生器] 以及行動應用程式。 這樣可讓主管和商務使用者使用他們可能已針對Adobe Analytics使用的相同應用程式，根據Customer Journey Analytics資料檢視其跨管道KPI。 |
...
| CUSTOMER JOURNEY ANALYTICSAPI | 2021 年 2 月 18 日 | Customer Journey Analytics API現已可用。 這些 API 可讓您以程式設計方式編輯元件及擷取報告。請參閱 [Customer Journey Analytics API檔案](https://www.adobe.io/cja-apis/docs) 以取得詳細資訊。 |
