...
* **開始推出**：分階段的發佈推出，從有限測試階段開始。 在隨後的幾個月內，該版本對客戶的可用性會從 0% 擴充至 100%。 在 CX Enterprise 組織層級進行分階段推出，因此組織中所有具備權限的使用者都會獲得相同的體驗。

* **正式發行 (GA)**：功能已開放供所有具有權限的 CX Enterprise 組織使用，且功能已發行完成。
