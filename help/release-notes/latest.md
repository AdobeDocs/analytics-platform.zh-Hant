...
| **B2B 查詢資料集的轉換** | 現在起，您可以在定義連線時[轉換 B2B 查詢資料集](/help/connections/transform-datasets-b2b-lookups.md)了。此轉換支援在 B2B 資料上進行人員型查詢。 | | 2024 年 6 月 6 日 |
...
| **衍生欄位 - 新函數和函數範本** | 其他衍生欄位函數 ([數學](/help/data-views/derived-fields/derived-fields.md#math)、[下一個或上一個](/help/data-views/derived-fields/derived-fields.md#next-or-previous)) 和[函數範本](/help/data-views/derived-fields/derived-fields.md#function-templates)。 | | 2024 年 6 月 5 日 |
| **共用帳戶和位置 (用於匯出和匯入)** | 使用者現在可以將其建立的帳戶和位置提供給組織中的所有使用者。只有帳戶和位置擁有者以及系統管理員才能編輯和刪除帳戶與位置。<p>先前，帳戶和位置只能由建立它們的使用者使用。</p><p>當使用者設定雲端匯出帳戶和設定雲端匯出位置時，可使用這些設定。</p> <p>如需詳細資訊，請參閱 [設定雲端匯出帳戶](/help/components/exports/cloud-export-accounts.md) 和 [設定雲端匯出位置](/help/components/exports/cloud-export-locations.md).</p> | 2024 年 6 月 12 日 | 2024 年 6 月 30 日 |
...
| **有關資料匯出使用案例的新文件** | 這個新的區段概述[資料匯出使用案例](https://experienceleague.adobe.com/zh-hant/docs/analytics-platform/using/cja-usecases/data-export/overview)，例如<ul><li>資料備份</li><li>資料驗證</li><li>資料湖、Data Warehouse 或 BI 工具</li><li>為 AI/ML 做好準備</li></ul> 以及如何使用 Experience Platform 和 Customer Journey Analytics 功能來實施。 | | 現已提供 |
